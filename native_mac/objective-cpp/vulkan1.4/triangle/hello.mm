// fork from https://vulkan-tutorial.com/Drawing_a_triangle/Swap_chain_recreation
...
static bool containsPathEntry(const char* list, const char* entry) {
	...
	while (*p) {
		...
		if (len == n && std::strncmp(p, entry, n) == 0) {
			return true;
		}
		if (!end) {
			break;
...
static void ensureLaunchEnvAndRelaunchIfNeeded(int argc, char** argv) {
	(void)argc;
	const char* guard = std::getenv("HELLO_VK_ENV_READY");
	if (guard && std::strcmp(guard, "1") == 0) {
		return;
	}
	...
	if (current == nullptr || current[0] == '\0') {
		...
		std::string merged = std::string(current) + ":" + defaults;
		setenv("DYLD_FALLBACK_LIBRARY_PATH", merged.c_str(), 1);
	}
	setenv("HELLO_VK_ENV_READY", "1", 1);
	execv(argv[0], argv);
}
...
private:
	...
	void initWindow() {
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Hello Vulkan (Objective-C++, MoltenVK)", nullptr, nullptr);
	...
	void createInstance() {
		if (enableValidationLayers && !checkValidationLayerSupport()) {
			std::cerr << "validation layers not available, disabling..." << std::endl;
	...
	void createLogicalDevice() {
		...
		std::vector<const char*> requiredDeviceExtensions = deviceExtensions;
	...
	std::vector<const char*> getRequiredExtensions() {
		...
		if (glfwExtensions == nullptr || glfwExtensionCount == 0) {
			throw std::runtime_error("GLFW Vulkan surface extensions are unavailable. Please install/build GLFW with Vulkan support.");
		}
