...
// macOS (GLFW + MoltenVK) + Manual Vulkan Bindings
// Compile: sh build.sh
...
extern(C) {
    ...
    VkResult     glfwCreateWindowSurface(VkInstance inst, GLFWwindow* win, const(void)* alloc, VkSurfaceKHR* surf);
}
...
extern(C) {
    ...
    alias VkInstanceCreateFlagBits = uint;
...
// MoltenVK/macOS portability extension names
...
extern(C) {
    ...
    alias PFN_vkDebugUtilsMessengerCallbackEXT = extern(C) VkBool32 function(
...
extern(C) {
    ...
    alias PFN_vkEnumerateInstanceExtensionProperties = VkResult function(const(char)*, uint*, VkExtensionProperties*);
...
uint[] readSpvFile(const(char)* filename) {
    ...
    auto buf = new uint[cast(size_t)((size + 3) / 4)];
    fread(buf.ptr, 1, cast(size_t)size, f);
    fclose(f);
    return buf;
}
...
// =============================================================================
// Helper: check if an instance extension is available
// =============================================================================

bool isInstanceExtensionAvailable(const(char)* name) {
    uint count = 0;
    vkEnumerateInstanceExtensionProperties(null, &count, null);
    auto props = allocArray!VkExtensionProperties(count);
    vkEnumerateInstanceExtensionProperties(null, &count, props);
    bool found = false;
    for (uint i = 0; i < count; i++) {
        if (strcmp(props[i].extensionName.ptr, name) == 0) {
            found = true;
            break;
        }
    }
...
// Helper: check if a device extension is available
...
void createInstance() {
    ...
    // Collect extensions: start from GLFW requirements
    uint glfwExtCount = 0;
    const(char)** glfwExts = glfwGetRequiredInstanceExtensions(&glfwExtCount);
    if (glfwExtCount == 0 || glfwExts is null) {
        printf("GLFW Vulkan extensions unavailable\n");
        exit(1);
    }

    // Gather into a dynamic array so we can append
    const(char)*[] extensions;
    for (uint i = 0; i < glfwExtCount; i++) {
        extensions ~= glfwExts[i];
    }

    // MoltenVK requires portability enumeration
    bool hasPortability = isInstanceExtensionAvailable(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME.ptr);
    if (hasPortability) {
        extensions ~= VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME.ptr;
    }
...
void createSurface() {
    if (glfwCreateWindowSurface(gApp.instance, gApp.window, null, &gApp.surface) != VK_SUCCESS) {
...
void createLogicalDevice() {
    ...
    // Build device extension list; always include swapchain
    const(char)*[] devExts;
    devExts ~= "VK_KHR_swapchain";
    // MoltenVK exposes VK_KHR_portability_subset — add it when present
    if (isDeviceExtensionAvailable(gApp.physicalDevice, VK_KHR_PORTABILITY_SUBSET_EXTENSION_NAME.ptr)) {
        devExts ~= VK_KHR_PORTABILITY_SUBSET_EXTENSION_NAME.ptr;
    }

    VkDeviceCreateInfo createInfo;
    createInfo.queueCreateInfoCount = queueCount;
    createInfo.pQueueCreateInfos = queueCreateInfos.ptr;
    createInfo.enabledExtensionCount = cast(uint)devExts.length;
...
VkExtent2D chooseSwapExtent(const(VkSurfaceCapabilitiesKHR)* caps) {
    ...
    int fbw = 0, fbh = 0;
    glfwGetFramebufferSize(gApp.window, &fbw, &fbh);
    VkExtent2D extent = VkExtent2D(cast(uint)fbw, cast(uint)fbh);
...
void createGraphicsPipeline() {
    // Load SPIR-V bytecode from compiled files
    uint[] vertCode = readSpvFile("hello_vert.spv");
    uint[] fragCode = readSpvFile("hello_frag.spv");
    ...
    vertStageInfo._module = vertShaderModule;
    vertStageInfo.pName   = "main";

    VkPipelineShaderStageCreateInfo fragStageInfo;
    fragStageInfo.stage   = VK_SHADER_STAGE_FRAGMENT_BIT;
    fragStageInfo._module = fragShaderModule;
    fragStageInfo.pName   = "main";

    VkPipelineShaderStageCreateInfo[2] shaderStages = [vertStageInfo, fragStageInfo];
    ...
    multisampling.rasterizationSamples = 1; // VK_SAMPLE_COUNT_1_BIT
...
void cleanup() {
    ...
    if (gVulkanLib !is null) dlclose(gVulkanLib);
...
void initWindow() {
    ...
    gApp.window = glfwCreateWindow(WIDTH, HEIGHT, "Hello, World!", null, null);
    if (gApp.window is null) {
