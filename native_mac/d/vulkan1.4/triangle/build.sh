...
LIBS="-L${VULKAN_LOADER_PREFIX}/lib -L${GLFW_PREFIX}/lib -link-defaultlib-shared -lglfw -lvulkan"

if command -v dmd >/dev/null 2>&1; then
    dmd -of=hello hello.d $INCLUDES $LIBS
elif command -v ldc2 >/dev/null 2>&1; then
    ldc2 -of=hello hello.d $INCLUDES $LIBS
elif command -v gdc >/dev/null 2>&1; then
    gdc -o hello hello.d -I${VULKAN_HEADERS_PREFIX}/include -I${GLFW_PREFIX}/include -L${VULKAN_LOADER_PREFIX}/lib -L${GLFW_PREFIX}/lib -lglfw -lvulkan
else
