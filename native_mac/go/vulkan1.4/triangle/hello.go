...
// macOS + GLFW + vulkan-go bindings + MoltenVK
// Build: sh build.sh
...
// Vertex shader (compiled from hello.vert with glslangValidator -V)
...
// Fragment shader (compiled from hello.frag with glslangValidator -V)
...
func (a *App) createInstance() {
	...
	// MoltenVK requires portability enumeration extension on macOS
	exts = append(exts, safeStr("VK_KHR_portability_enumeration"))
...
func (a *App) createLogicalDevice() {
	...
	// VK_KHR_portability_subset is required on MoltenVK (macOS)
	deviceExts := safeStrSlice([]string{"VK_KHR_swapchain", "VK_KHR_portability_subset"})
