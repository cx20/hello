...
static bool isInstanceExtensionAvailable(const char *name) {
    ...
    vkEnumerateInstanceExtensionProperties(NULL, &extensionCount, available);
    bool found = false;
...
static void initWindow(App *app) {
    ...
    app->window = glfwCreateWindow(WIDTH, HEIGHT, "Hello Vulkan (Objective-C, MoltenVK)", NULL, NULL);
    glfwSetWindowUserPointer(app->window, app);
    glfwSetFramebufferSizeCallback(app->window, framebufferResizeCallback);
}

/* macOS/MoltenVK: VK_KHR_portability_enumeration extension is required */
static const char **getRequiredExtensions(uint32_t *extensionCount, bool *enablePortabilityEnumeration) {
    uint32_t glfwExtensionCount = 0;
    const char **glfwExtensions = glfwGetRequiredInstanceExtensions(&glfwExtensionCount);

    if (glfwExtensionCount == 0 || glfwExtensions == NULL) {
        fprintf(stderr, "GLFW Vulkan surface extensions are unavailable.\n");
        fprintf(stderr, "Please install/build GLFW with Vulkan support.\n");
        exit(EXIT_FAILURE);
    }

    bool hasPortabilityEnumeration = isInstanceExtensionAvailable(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
    /* +1 for portability enumeration if available, +1 for debug utils if enabled */
    uint32_t extra = (hasPortabilityEnumeration ? 1 : 0) + (enableValidationLayers ? 1 : 0);
    uint32_t total = glfwExtensionCount + extra;
    const char **extensions = malloc(sizeof(char *) * total);
    for (uint32_t i = 0; i < glfwExtensionCount; i++) {
        extensions[i] = glfwExtensions[i];
    }
    uint32_t idx = glfwExtensionCount;
    ...
    *enablePortabilityEnumeration = hasPortabilityEnumeration;
...
static void createInstance(App *app) {
    /* macOS: validation layers may not be available without LunarG SDK */
    if (enableValidationLayers && !checkValidationLayerSupport()) {
        fprintf(stderr, "validation layers not available, disabling...\n");
    ...
    uint32_t extensionCount = 0;
    const char **extensions = getRequiredExtensions(&extensionCount, &enablePortabilityEnumeration);
...
/* macOS/MoltenVK: check whether VK_KHR_portability_subset is exposed */
...
static void createLogicalDevice(App *app) {
    ...
    /* macOS/MoltenVK: add VK_KHR_portability_subset if exposed by the driver */
    const char *finalExtensions[DEVICE_EXTENSION_COUNT + 1];
    uint32_t finalCount = DEVICE_EXTENSION_COUNT;
    for (uint32_t i = 0; i < DEVICE_EXTENSION_COUNT; i++) {
        finalExtensions[i] = DEVICE_EXTENSIONS[i];
    }
    if (isDeviceExtensionAvailable(app->physicalDevice, "VK_KHR_portability_subset")) {
        finalExtensions[finalCount++] = "VK_KHR_portability_subset";
    }
    ...
    createInfo.enabledExtensionCount = finalCount;
