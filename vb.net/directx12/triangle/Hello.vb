...
Public Class Hello
	...
	Private Sub WaitForPreviousFrame()
		...
		Console.WriteLine("[WaitForPreviousFrame] - Start")
		Try
			Dim ptr As IntPtr = Marshal.ReadIntPtr(Me.commandQueue)
			Dim ptr2 As IntPtr = Marshal.ReadIntPtr(ptr, 14 * System.IntPtr.Size)
			Dim delegateForFunctionPointer As Hello.SignalDelegate = Marshal.GetDelegateForFunctionPointer(Of Hello.SignalDelegate)(ptr2)
			Dim num As Integer = delegateForFunctionPointer(Me.commandQueue, Me.fence, Me.fenceValue)
			Dim flag As Boolean = num < 0
			If flag Then
				Console.WriteLine(String.Format("Signal failed with HRESULT: {0:X}", num))
			Else
				Dim ptr3 As IntPtr = Marshal.ReadIntPtr(Me.fence)
				Dim ptr4 As IntPtr = Marshal.ReadIntPtr(ptr3, 8 * IntPtr.Size)
				Dim delegateForFunctionPointer2 As Hello.GetCompletedValueDelegate = Marshal.GetDelegateForFunctionPointer(Of Hello.GetCompletedValueDelegate)(ptr4)
				Dim flag2 As Boolean = delegateForFunctionPointer2(Me.fence) < Me.fenceValue
				If flag2 Then
					Dim ptr5 As IntPtr = Marshal.ReadIntPtr(ptr3, 9 * IntPtr.Size)
					Dim delegateForFunctionPointer3 As Hello.SetEventOnCompletionDelegate = Marshal.GetDelegateForFunctionPointer(Of Hello.SetEventOnCompletionDelegate)(ptr5)
					num = delegateForFunctionPointer3(Me.fence, Me.fenceValue, Me.fenceEvent)
					Dim flag3 As Boolean = num < 0
					If flag3 Then
						Console.WriteLine(String.Format("SetEventOnCompletion failed with HRESULT: {0:X}", num))
						Return
					End If
					Hello.WaitForSingleObject(Me.fenceEvent, 4294967295UI)
				End If
				Me.fenceValue += 1UL
			End If
		Catch ex As Exception
	...
	Public Sub Render()
		...
		Try
			...
			Dim ptr As IntPtr = Marshal.ReadIntPtr(Me.commandQueue)
			Dim ptr2 As IntPtr = Marshal.ReadIntPtr(ptr, 10 * System.IntPtr.Size)
			Dim delegateForFunctionPointer As Hello.ExecuteCommandListsDelegate = Marshal.GetDelegateForFunctionPointer(Of Hello.ExecuteCommandListsDelegate)(ptr2)
			delegateForFunctionPointer(Me.commandQueue, 1UI, ppCommandLists)
			ptr = Marshal.ReadIntPtr(Me.swapChain)
			Dim ptr3 As IntPtr = Marshal.ReadIntPtr(ptr, 8 * IntPtr.Size)
			Dim delegateForFunctionPointer2 As Hello.PresentDelegate = Marshal.GetDelegateForFunctionPointer(Of Hello.PresentDelegate)(ptr3)
			Dim num As Integer = delegateForFunctionPointer2(Me.swapChain, 0UI, 0UI)
			Dim flag As Boolean = num < 0
			If flag Then
