...
class HelloDirect2D
{
    ...
    static void Draw()
    {
        ...
        endDraw(g_renderTarget, out _, out _);
    }
    ...
    static bool InitDirect2D(IntPtr hWnd)
    {
        ...
        IntPtr createHwndRTPtr = Marshal.ReadIntPtr(vt, 14 * IntPtr.Size);  // #14 ID2D1Factory::CreateHwndRenderTarget
        ...
        // CreateSolidColorBrush (blue)
        vt = Marshal.ReadIntPtr(g_renderTarget);
        IntPtr createBrushPtr = Marshal.ReadIntPtr(vt, 8 * IntPtr.Size);  // #8 ID2D1RenderTarget::CreateSolidColorBrush
    ...
    static void Cleanup()
    {
        ...
        Release(ref g_brush);
        Release(ref g_renderTarget);
        Release(ref g_factory);
        if (g_hD2D1 != IntPtr.Zero) FreeLibrary(g_hD2D1);
    }

    [STAThread]
    static int Main()
    {
        IntPtr hInstance = Marshal.GetHINSTANCE(typeof(HelloDirect2D).Module);
        ...
        IntPtr hWnd = CreateWindowEx(0, wcex.lpszClassName, "Hello, Direct2D(C#) World!",
