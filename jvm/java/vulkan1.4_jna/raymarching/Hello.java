...
public class Hello {
    ...
    public interface User32 extends StdCallLibrary {
        ...
        Pointer CreateWindowExW(int dwExStyle, WString lpClassName, WString lpWindowName, int dwStyle,
    ...
    // JNA Vulkan loader
    // ============================================================
    static final NativeLibrary VK_LIB = NativeLibrary.getInstance("vulkan-1");
    static final Function vkGetInstanceProcAddr = VK_LIB.getFunction("vkGetInstanceProcAddr", Function.ALT_CONVENTION);
    static final Function vkGetDeviceProcAddr = VK_LIB.getFunction("vkGetDeviceProcAddr", Function.ALT_CONVENTION);

    static Function vkGlobal(String name) {
        return VK_LIB.getFunction(name, Function.ALT_CONVENTION);
    }
    ...
    public static class VkInstanceCreateInfo extends Structure {
        ...
        public Pointer pNext;
        public int flags;
        public Pointer pApplicationInfo;
    ...
    public static class VkSurfaceFormatKHR extends Structure {
        ...
        @Override
        protected List<String> getFieldOrder() { return Arrays.asList("format", "colorSpace"); }
    }
    ...
    public static class VkImageSubresourceRange extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("aspectMask", "baseMipLevel", "levelCount", "baseArrayLayer", "layerCount");
        }
    }
    ...
    public static class VkPipelineInputAssemblyStateCreateInfo extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("sType", "pNext", "flags", "topology", "primitiveRestartEnable");
        }
    }
    ...
    public static class VkViewport extends Structure {
        ...
        public VkViewport() {}
        public VkViewport(float x, float y, float w, float h, float minD, float maxD) {
            this.x = x; this.y = y; this.width = w; this.height = h; this.minDepth = minD; this.maxDepth = maxD;
        }

        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("x", "y", "width", "height", "minDepth", "maxDepth");
        }
    }
    ...
    public static class VkDynamicStateCreateInfo extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("sType", "pNext", "flags", "dynamicStateCount", "pDynamicStates");
        }
    }

    // Push constant range structure
    public static class VkPushConstantRange extends Structure {
        public int stageFlags;
        public int offset;
        public int size;

        public VkPushConstantRange() {}
        public VkPushConstantRange(int stageFlags, int offset, int size) {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("stageFlags", "offset", "size");
        }
    }
    ...
    public static class VkCommandPoolCreateInfo extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("sType", "pNext", "flags", "queueFamilyIndex");
        }
    }
    ...
    public static class VkCommandBufferAllocateInfo extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("sType", "pNext", "commandPool", "level", "commandBufferCount");
        }
    }
    ...
    public static class VkCommandBufferBeginInfo extends Structure {
        ...
        @Override protected List<String> getFieldOrder() {
            return Arrays.asList("sType", "pNext", "flags", "pInheritanceInfo");
        }
    }
    ...
    static Pointer g_hInstance;
    static boolean g_running = true;
    static boolean g_resized = false;
    ...
    // Device functions (used every frame)
    ...
    // Device functions for per-frame command recording
    static Function vkResetCommandBuffer;
    static Function vkBeginCommandBuffer;
    static Function vkEndCommandBuffer;
    static Function vkCmdBeginRenderPass;
    static Function vkCmdEndRenderPass;
    static Function vkCmdBindPipeline;
    static Function vkCmdDraw;
    static Function vkCmdSetViewport;
    static Function vkCmdSetScissor;
    static Function vkCmdPushConstants;
    ...
    // Window Procedure
    // ============================================================
    static WndProc wndProc = (hwnd, uMsg, wParam, lParam) -> {
        switch (uMsg) {
            case WM_DESTROY:
    ...
    public static void main(String[] args) throws Exception {
        // Load SPIR-V
        vertSpv = Files.readAllBytes(Paths.get("hello_vert.spv"));
        fragSpv = Files.readAllBytes(Paths.get("hello_frag.spv"));
        ...
        createSwapchainBundle();
        createSyncObjects();

        // Record start time for animation
        startTimeNano = System.nanoTime();

        System.out.println("Vulkan initialized. Entering main loop...");
    ...
    static void createWindow() {
        g_hInstance = Kernel32.INSTANCE.GetModuleHandleW(null);

        WString className = new WString("VulkanWindowClass");
        ...
        wc.hIcon = null;
        wc.hCursor = User32.INSTANCE.LoadCursorW(null, new Pointer(IDC_ARROW));
        wc.hbrBackground = new Pointer(COLOR_WINDOW + 1);
        ...
        if (atom == 0) {
            throw new RuntimeException("RegisterClassExW failed");
        }

        g_hWnd = User32.INSTANCE.CreateWindowExW(
                0,
                className,
                new WString("Vulkan 1.4 Raymarching (JNA only)"),
        ...
        User32.INSTANCE.ShowWindow(g_hWnd, SW_SHOW);
        User32.INSTANCE.UpdateWindow(g_hWnd);

        System.out.println("Window created: " + g_hWnd);
    }
    ...
    static void createInstance() {
        ...
        app.pNext = null;
        Memory appName = utf8z("VulkanRaymarching");
    ...
    static void createSurface() {
        ...
        System.out.println("Surface created: " + surface);
    }

    static void pickPhysicalDeviceAndQueues() {
        Function vkEnumeratePhysicalDevices = vkInst(instance, "vkEnumeratePhysicalDevices");
        Function vkGetPhysicalDeviceQueueFamilyProperties = vkInst(instance, "vkGetPhysicalDeviceQueueFamilyProperties");
    ...
    static void createDeviceAndQueues() {
        ...
        Memory qciBuf = new Memory((long) qci0.size() * qf.length);
        ...
        dci.pNext = null;
        dci.flags = 0;
        dci.queueCreateInfoCount = qf.length;
    ...
    static void loadLoopFunctions() {
        ...
        // Functions for per-frame command recording
        vkResetCommandBuffer = vkDev(device, "vkResetCommandBuffer");
        vkBeginCommandBuffer = vkDev(device, "vkBeginCommandBuffer");
        vkEndCommandBuffer = vkDev(device, "vkEndCommandBuffer");
        vkCmdBeginRenderPass = vkDev(device, "vkCmdBeginRenderPass");
        vkCmdEndRenderPass = vkDev(device, "vkCmdEndRenderPass");
        vkCmdBindPipeline = vkDev(device, "vkCmdBindPipeline");
        vkCmdDraw = vkDev(device, "vkCmdDraw");
        vkCmdSetViewport = vkDev(device, "vkCmdSetViewport");
        vkCmdSetScissor = vkDev(device, "vkCmdSetScissor");
        vkCmdPushConstants = vkDev(device, "vkCmdPushConstants");
    }

    // ============================================================
    // Swapchain Bundle
    // ============================================================
    static void createSwapchainBundle() {
        ...
        for (int i = 0; i < fc; i++) {
            ...
            f.read();
            if (f.format == 44) { // VK_FORMAT_B8G8R8A8_UNORM
        ...
        RECT rc = new RECT();
        User32.INSTANCE.GetClientRect(g_hWnd, rc);
        ...
        Memory imgsBuf = new Memory(8L * swapchainImageCount);
    ...
    static void createPipeline() {
        ...
        // Push constant range: iTime(float) + padding(float) + iResolution(vec2) = 16 bytes
        VkPushConstantRange pcRange = new VkPushConstantRange(VK_SHADER_STAGE_FRAGMENT_BIT, 0, 16);
        pcRange.write();
        ...
        plci.pPushConstantRanges = pcRange.getPointer();
        plci.write();
    ...
    // ============================================================
    // Record command buffer for a single frame (with push constants)
    // ============================================================
    static void recordCommandBuffer(Pointer cmd, int imageIndex, float currentTime) {
        // Reset and begin command buffer
        vkResetCommandBuffer.invokeInt(new Object[]{cmd, 0});
        ...
        vkCheck(vkBeginCommandBuffer.invokeInt(new Object[]{cmd, bi.getPointer()}), "vkBeginCommandBuffer");

        // Clear value (black background)
        ...
        vkCmdBeginRenderPass.invokeVoid(new Object[]{cmd, rpbi.getPointer(), 0});

        // Bind pipeline
        vkCmdBindPipeline.invokeVoid(new Object[]{cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline});

        // Set viewport and scissor
        VkViewport vpStruct = new VkViewport(0, 0, swapchainExtent.width, swapchainExtent.height, 0, 1);
        vpStruct.write();
        VkRect2D sc = new VkRect2D(new VkOffset2D(0, 0), swapchainExtent);
        sc.write();

        vkCmdSetViewport.invokeVoid(new Object[]{cmd, 0, 1, vpStruct.getPointer()});
        vkCmdSetScissor.invokeVoid(new Object[]{cmd, 0, 1, sc.getPointer()});

        // Push constants: iTime(float), padding(float), iResolution.x(float), iResolution.y(float) = 16 bytes
        Memory pcData = new Memory(16);
        pcData.setFloat(0, currentTime);                          // iTime
        pcData.setFloat(4, 0.0f);                                 // padding
        pcData.setFloat(8, (float) swapchainExtent.width);        // iResolution.x
        pcData.setFloat(12, (float) swapchainExtent.height);      // iResolution.y

        vkCmdPushConstants.invokeVoid(new Object[]{cmd, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 16, pcData});

        // Draw fullscreen triangle (3 vertices, defined in vertex shader)
        vkCmdDraw.invokeVoid(new Object[]{cmd, 3, 1, 0, 0});

        // End render pass and command buffer
        vkCmdEndRenderPass.invokeVoid(new Object[]{cmd});

        vkCheck(vkEndCommandBuffer.invokeInt(new Object[]{cmd}), "vkEndCommandBuffer");
    }
    ...
    static void renderFrame() {
        ...
        vkCheck(vkWaitForFences.invokeInt(new Object[]{device, 1, fences, 1, -1L}), "vkWaitForFences");
        ...
        float currentTime = (float) ((System.nanoTime() - startTimeNano) / 1_000_000_000.0);

        // Record command buffer with push constants for this frame
        recordCommandBuffer(commandBuffers[imageIndex.getValue()], imageIndex.getValue(), currentTime);

        Memory waitSems = new Memory(8);
        waitSems.setLong(0, imageAvailable[cur]);
        Memory signalSems = new Memory(8);
        signalSems.setLong(0, renderFinished[cur]);
        Memory waitStages = new Memory(4);
        waitStages.setInt(0, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
        ...
        vkCheck(vkQueueSubmit.invokeInt(new Object[]{graphicsQueue, 1, si.getPointer(), inFlightFences[cur]}), "vkQueueSubmit");

        Memory swapchains = new Memory(8);
        swapchains.setLong(0, swapchain);
        Memory indices = new Memory(4);
        indices.setInt(0, imageIndex.getValue());
