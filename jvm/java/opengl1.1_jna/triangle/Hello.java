...
public class Hello {
    ...
    // Globals
    // -----------------------------
    private static final String CLASS_NAME = "HelloJnaOpenGL11Class";
    ...
    private static Memory colorsMemory;
    private static Memory verticesMemory;
    ...
    private static void initVertexData() {
        ...
        colorsMemory = new Memory(colors.length * 4);
        for (int i = 0; i < colors.length; i++) {
            colorsMemory.setFloat(i * 4, colors[i]);
        }

        verticesMemory = new Memory(vertices.length * 4);
        for (int i = 0; i < vertices.length; i++) {
            verticesMemory.setFloat(i * 4, vertices[i]);
        }
    }

    private static void drawTriangle() {
        OpenGL32.INSTANCE.glEnableClientState(GL_COLOR_ARRAY);
        OpenGL32.INSTANCE.glEnableClientState(GL_VERTEX_ARRAY);

        OpenGL32.INSTANCE.glColorPointer(3, GL_FLOAT, 0, colorsMemory);
        OpenGL32.INSTANCE.glVertexPointer(2, GL_FLOAT, 0, verticesMemory);

        OpenGL32.INSTANCE.glDrawArrays(GL_TRIANGLE_STRIP, 0, 3);

        OpenGL32.INSTANCE.glDisableClientState(GL_VERTEX_ARRAY);
        OpenGL32.INSTANCE.glDisableClientState(GL_COLOR_ARRAY);
    }
}
