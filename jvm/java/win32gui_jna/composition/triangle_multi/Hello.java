// Hello.java
// OpenGL 4.6 + D3D11 + Vulkan 1.4 Triangles via DirectComposition
// ALL COM calls via vtable index. Java + JNA only.
//
// Build & Run:
...
//
// Requirements:
//   - Windows 8+ with DirectComposition support
//   - D3D11, OpenGL 4.6, Vulkan 1.4 capable GPU
...
//   - jna.jar (JNA 5.13+)
//
// forked from Hello.cs (C# DirectComposition vtable version)
...
public class Hello {
    ...
    public interface DcompDll extends StdCallLibrary {
        DcompDll INSTANCE = Native.load("dcomp", DcompDll.class);
        int DCompositionCreateDevice(Pointer dxgiDevice, Pointer iid, PointerByReference dcompDevice);
    }
    ...
    // IDCompositionDevice GUID
    static final GUID IID_IDCompositionDevice = guid("C37EA93A-E7AA-450D-B16F-9746CB0407F3");
    ...
    static final int DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL = 3;
    static final int DXGI_ALPHA_MODE_PREMULTIPLIED = 1;
    ...
    // DirectComposition COM pointers
    static Pointer g_dcompDevice;       // IDCompositionDevice
    static Pointer g_dcompTarget;       // IDCompositionTarget
    static Pointer g_dcompRootVisual;   // IDCompositionVisual (root)
    static Pointer[] g_dcompVisual = new Pointer[PANEL_COUNT]; // IDCompositionVisual (children)
    ...
    static Pointer createAppWindow() {
        ...
        WString className = new WString("DCompMultiJava");
        ...
        Pointer hwnd = User32Lib.INSTANCE.CreateWindowExW(0,
            className,
            new WString("OpenGL + D3D11 + Vulkan via DirectComposition (Java+JNA)"),
    ...
    // DirectComposition requires FLIP_SEQUENTIAL + PREMULTIPLIED alpha
    static Memory createSwapChainDesc() {
        ...
        m.setInt(36, DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL);     // SwapEffect
        m.setInt(40, DXGI_ALPHA_MODE_PREMULTIPLIED);        // AlphaMode
        m.setInt(44, 0);                                    // Flags
        return m;
    }
    ...
    static void initVulkan(Pointer swapChain) throws Exception {
        ...
        // Create offscreen image (B8G8R8A8_UNORM = format 44 to match D3D11)
        ...
        stages.setInt(20, 1);                // stage = VERTEX_BIT (offset 20)
        ...
        iaCI.setInt(0, 20);
        iaCI.setInt(20, 3);  // topology = TRIANGLE_LIST (offset 20)
        ...
        vpCI.setInt(0, 22);
        vpCI.setInt(20, 1);           // viewportCount (offset 20)
        ...
        msCI.setInt(0, 24);
        msCI.setInt(20, 1);            // rasterizationSamples = 1 (offset 20)
        ...
        cbCI.setInt(0, 26);
        cbCI.setInt(28, 1);            // attachmentCount (offset 28)
        cbCI.setPointer(32, cbAtt);    // pAttachments (offset 32)
        ...
        gpCI.setInt(136, -1);                  // basePipelineIndex
        ...
        // Fence (signaled)
        // VkFenceCreateInfo: sType(0) pad(4) pNext(8) flags(16) -> 24 bytes
        Memory fCI = new Memory(24); fCI.clear();
        fCI.setInt(0, 8);                    // sType
        fCI.setInt(16, 1);                   // flags = SIGNALED (offset 16)
    ...
    //   Sets up DirectComposition visual tree with 3 panels
    //
    //   IDCompositionDevice vtable:
    //     3 = Commit
    //     6 = CreateTargetForHwnd(hwnd, topmost, &target)
    //     7 = CreateVisual(&visual)
    //
    //   IDCompositionTarget vtable:
    //     3 = SetRoot(visual)
    //
    //   IDCompositionVisual vtable:
    //     4  = SetOffsetX(float)
    //     6  = SetOffsetY(float)
    //     15 = SetContent(pUnknown)  [swap chain]
    //     16 = AddVisual(child, insertAbove, refVisual)
    // ============================================================
    static Pointer g_glSwapChain, g_vkSwapChain;

    static int addDCompPanel(Pointer swapChain, float offsetX, int index) {
        final String FN = "AddDCompPanel[" + index + "]";

        // IDCompositionDevice::CreateVisual (vt#7)
        PointerByReference pVis = new PointerByReference();
        int hr = comCall(g_dcompDevice, 7, pVis);
        if (hr < 0) { dbgHR(FN, "CreateVisual", hr); return hr; }
        g_dcompVisual[index] = pVis.getValue();

        // IDCompositionVisual::SetContent (vt#15) - set swap chain as content
        hr = comCall(g_dcompVisual[index], 15, swapChain);
        if (hr < 0) { dbgHR(FN, "SetContent", hr); return hr; }

        // IDCompositionVisual::SetOffsetX (vt#4) - float overload
        hr = comCall(g_dcompVisual[index], 4, offsetX);
        if (hr < 0) { dbgHR(FN, "SetOffsetX", hr); return hr; }

        // IDCompositionVisual::SetOffsetY (vt#6) - float overload
        hr = comCall(g_dcompVisual[index], 6, 0.0f);
        if (hr < 0) { dbgHR(FN, "SetOffsetY", hr); return hr; }

        // IDCompositionVisual::AddVisual (vt#16) on root visual
        // AddVisual(child, insertAbove=TRUE, refVisual=NULL)
        hr = comCall(g_dcompRootVisual, 16, g_dcompVisual[index], 1, Pointer.NULL);
        if (hr < 0) { dbgHR(FN, "AddVisual", hr); return hr; }

        dbg(FN, "ok");
        return 0;
    }

    static int initDirectComposition() {
        final String FN = "InitDirectComposition";
        dbg(FN, "begin");
        ...
        // DCompositionCreateDevice(pDXGIDevice, IID_IDCompositionDevice, &pDevice)
        PointerByReference pDComp = new PointerByReference();
        hr = DcompDll.INSTANCE.DCompositionCreateDevice(
            pDxgiDev.getValue(), guidToMemory(IID_IDCompositionDevice), pDComp);
        comRelease(pDxgiDev.getValue());
        if (hr < 0) { dbgHR(FN, "DCompositionCreateDevice", hr); return hr; }
        g_dcompDevice = pDComp.getValue();
        dbgPtr(FN, "DCompDevice", g_dcompDevice);

        // IDCompositionDevice::CreateTargetForHwnd (vt#6)
        // CreateTargetForHwnd(hwnd, topmost=TRUE, &target)
        PointerByReference pTarget = new PointerByReference();
        hr = comCall(g_dcompDevice, 6, g_hwnd, 1, pTarget);
        if (hr < 0) { dbgHR(FN, "CreateTargetForHwnd", hr); return hr; }
        g_dcompTarget = pTarget.getValue();

        // IDCompositionDevice::CreateVisual (vt#7) for root
        PointerByReference pRoot = new PointerByReference();
        hr = comCall(g_dcompDevice, 7, pRoot);
        if (hr < 0) { dbgHR(FN, "CreateVisual(root)", hr); return hr; }
        g_dcompRootVisual = pRoot.getValue();

        // IDCompositionTarget::SetRoot (vt#3)
        hr = comCall(g_dcompTarget, 3, g_dcompRootVisual);
        if (hr < 0) { dbgHR(FN, "SetRoot", hr); return hr; }

        // Add three composition panels: OpenGL, D3D11, Vulkan
        hr = addDCompPanel(g_glSwapChain,  0.0f, 0);
        if (hr < 0) { dbgHR(FN, "AddPanel(GL)", hr); return hr; }
        hr = addDCompPanel(g_d3dSwapChain, (float)WIDTH, 1);
        if (hr < 0) { dbgHR(FN, "AddPanel(D3D)", hr); return hr; }
        hr = addDCompPanel(g_vkSwapChain,  (float)WIDTH * 2, 2);
        if (hr < 0) { dbgHR(FN, "AddPanel(VK)", hr); return hr; }

        // IDCompositionDevice::Commit (vt#3) - must commit to apply changes
        hr = comCall(g_dcompDevice, 3);
        if (hr < 0) { dbgHR(FN, "Commit", hr); return hr; }
    ...
    static void renderVulkan() {
        ...
        region.setInt(44, WIDTH);     // imageExtent.width (offset 44)
        region.setInt(48, HEIGHT);    // imageExtent.height (offset 48)
        region.setInt(52, 1);         // imageExtent.depth (offset 52)
        ...
        si.setInt(0, 4);         // sType
        si.setInt(40, 1);        // commandBufferCount (offset 40)
        si.setPointer(48, cbPtrs); // pCommandBuffers (offset 48)
    ...
    static void cleanup() {
        ...
        for (int i = 0; i < PANEL_COUNT; i++) {
            comRelease(g_dcompVisual[i]);
        }
        comRelease(g_dcompRootVisual);
        comRelease(g_dcompTarget);
        comRelease(g_dcompDevice);
    ...
    public static void main(String[] args) {
        ...
        dbg(FN, "========================================");
        dbg(FN, "OpenGL 4.6 + D3D11 + Vulkan 1.4 via DirectComposition (Java+JNA vtable)");
        dbg(FN, "========================================");
        ...
        if (hr < 0) { dbg(FN, "FATAL: initDirectComposition failed"); cleanup(); return; }
