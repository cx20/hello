...
# ============================================================
print "=== Perl DirectX12 Raymarching via COM vtable (64-bit) ===\n\n";
...
use constant {
    ...
    ID3D12GraphicsCommandList_SetGraphicsRootConstantBufferView => 38,
...
sub load_hlsl_source {
    ...
    open my $fh, '<', $path or die "Failed to open hello.hlsl: $!\n";
    binmode $fh;
    local $/;
    my $src = <$fh>;
    close $fh;
...
# ============================================================
# Fullscreen triangle: position (2 floats) = 8 bytes
my @VERTICES = (
    { pos => [-1.0, -1.0] },
    { pos => [-1.0,  3.0] },
    { pos => [ 3.0, -1.0] },
...
my $VERTEX_BUFFER_SIZE = 3 * 8;  # 3 vertices, 8 bytes each
...
sub init_d3d {
    ...
    my $root_param_addr = alloc_work(24);
    my $root_param = pack('L L L L Q',
    ...
    write_mem($root_param_addr, $root_param);
    ...
    my $input_elem_addr = alloc_work(32);
    my $ie0 = pack('Q L L L L L L',
        $pos_sem_addr, 0, DXGI_FORMAT_R32G32_FLOAT, 0, 0,
        D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0
    );
    write_mem($input_elem_addr, $ie0);

    my $input_layout = pack('Q L x4', $input_elem_addr, 1);
    ...
    write_mem($g_vb_view_addr, pack('Q L L', $gpu_addr, $vb_size, 8));

    # Create constant buffer (256-byte aligned)
    debug_print("Creating constant buffer...");
    my $cb_size = 256;
    my $cb_res_desc = pack('L x4 Q Q L S S L L L L',
        D3D12_RESOURCE_DIMENSION_BUFFER, 0, $cb_size, 1,
        1, 1, 0, 1, 0, D3D12_TEXTURE_LAYOUT_ROW_MAJOR, D3D12_RESOURCE_FLAG_NONE
    );
    my $cb_res_desc_addr = alloc_work(length($cb_res_desc));
    write_mem($cb_res_desc_addr, $cb_res_desc);
    ...
    $g_cbv_buffer = read_ptr($cb_addr);
    $g_cbv_gpu_addr = com_call_0_ptr($g_cbv_buffer, ID3D12Resource_GetGPUVirtualAddress);

    my $cb_map_addr = read_ptr(read_ptr($g_cbv_buffer) + ID3D12Resource_Map * 8);
    ...
    $code = pack('H*', '4883EC28');
    $code .= pack('H*', '48B9') . pack('Q', $g_cbv_buffer);
    $code .= pack('H*', '48BA') . pack('Q', 0);
    ...
    $g_cbv_data_ptr = read_ptr($cb_data_addr);
    debug_print("Constant buffer created and mapped");

    # Viewport/scissor
    $g_viewport_addr = alloc_work(24);
    write_mem($g_viewport_addr, pack('f6', 0.0, 0.0, $g_width, $g_height, 0.0, 1.0));
    $g_scissor_addr = alloc_work(16);
    write_mem($g_scissor_addr, pack('l4', 0, 0, $g_width, $g_height));
...
sub render {
    ...
    if ($g_cbv_gpu_addr && $g_cbv_data_ptr) {
        my $t = time - $g_start_time;
        write_mem($g_cbv_data_ptr, pack('f4', $t, $g_width, $g_height, 0.0));
        com_call_2($g_command_list, ID3D12GraphicsCommandList_SetGraphicsRootConstantBufferView, 0, $g_cbv_gpu_addr);
    }
...
sub cleanup {
    ...
    com_call_0($g_cbv_buffer, IUnknown_Release) if $g_cbv_buffer;
...
eval {
    ...
    my $window_title = encode_utf16("DirectX12 Perl Raymarching");
    ...
    ($g_width, $g_height) = get_client_size($hwnd);
    print "Client size: ${g_width}x${g_height}\n";
    
    print "\n--- Initializing Direct3D 12 ---\n";
    init_d3d();
    
    print "\n=== DirectX12 Raymarching Demo ===\n";
