"""
OpenGL 4.6 Triangle via Windows.UI.Composition (ctypes only, no external libraries)

This sample demonstrates how to render an OpenGL 4.6 triangle through
Windows.UI.Composition on a classic desktop window, using only Python's
ctypes without any external packages.
...
Architecture:
  ...
  5. Use WGL_NV_DX_interop to share the D3D11 back buffer with OpenGL
  6. Render an OpenGL 4.6 triangle into the shared FBO
  7. Present via the swap chain; Composition displays it
...
def create_window():
    ...
    g_hwnd = user32.CreateWindowExW(
        ...
        class_name,
        "OpenGL 4.6 Triangle via Windows.UI.Composition (Python)",
        style,
...
# ============================================================
# Render: draw OpenGL triangle into shared FBO, present swap chain
# ============================================================
def render():
    if not g_hdc or not g_hglrc or not g_gl_interop_device or not g_gl_interop_object:
        return
    if not g_gl_fbo.value:
        return
    ...
    # Render into the shared FBO
    gl["glBindFramebuffer"](GL_FRAMEBUFFER, g_gl_fbo)
    opengl32.glViewport(0, 0, WIDTH, HEIGHT)
    opengl32.glClearColor(1.0, 1.0, 1.0, 1.0)  # White background
    ...
    # Unlock the interop object
    wgl["wglDXUnlockObjectsNV"](g_gl_interop_device, 1, objs)

    # Present the swap chain (Composition will pick it up)
    # IDXGISwapChain::Present (slot 8)
    fn = com_call(g_swap_chain.value, 8, wintypes.HRESULT,
...
def cleanup():
    ...
    for obj in [g_rtv, g_back_buffer, g_swap_chain, g_d3d_ctx, g_d3d_device]:
        com_release(obj)
...
# ============================================================
def main():
    debug_print("=== OpenGL 4.6 Triangle via Windows.UI.Composition (Python) ===")
    ...
    try:
        ...
        # Step 3: Initialize Composition (WinRT)
        init_composition()

        # Step 4: Initialize OpenGL 4.6 with NV_DX_interop
        init_opengl()
