...
' ============================================================
'  OpenGL 1.1 Triangle in Excel VBA (VBA7 / 64-bit safe)
'  - Creates a native Win32 window, sets pixel format,
'    creates WGL context and renders a triangle.
' ============================================================
...
Private Const GL_DOUBLE As Long = &H140A&
...
' Class / window names
' -----------------------------
Private Const CLASS_NAME As String = "helloWindowVBA"
Private Const WINDOW_NAME As String = "Hello, OpenGL (VBA)"
...
#Else
    ' (If you ever need pre-VBA7, add 32-bit declares here)
#End If

' ============================================================
' Window procedure (must be in a standard module)
' ============================================================
Public Function WindowProc(ByVal hWnd As LongPtr, ByVal uMsg As Long, ByVal wParam As LongPtr, ByVal lParam As LongPtr) As LongPtr
    Select Case uMsg
        ...
        Case WM_DESTROY
            ' Window destroyed -> quit loop
            PostQuitMessage 0
            WindowProc = 0
            Exit Function

        Case WM_SIZE
            ' Update viewport on resize (if GL context already created)
...
Private Function EnableOpenGL(ByVal hWnd As LongPtr, ByVal hDC As LongPtr) As LongPtr
    ...
    ' Fill PFD (matches Win32 PIXELFORMATDESCRIPTOR)
    pfd.nSize = LenB(pfd)
    pfd.nVersion = 1
...
Private Sub DrawTriangle()
    ' 3 vertices (x,y,z) and 3 colors (r,g,b)
    Dim Color(0 To 8) As Double
    Color(0) = 1#: Color(1) = 0#: Color(2) = 0#
    Color(3) = 0#: Color(4) = 1#: Color(5) = 0#
    Color(6) = 0#: Color(7) = 0#: Color(8) = 1#

    Dim Vertex(0 To 8) As Double
    Vertex(0) = 0#:    Vertex(1) = 0.6:   Vertex(2) = 0#
    Vertex(3) = 0.6:   Vertex(4) = -0.6: Vertex(5) = 0#
    Vertex(6) = -0.6: Vertex(7) = -0.6: Vertex(8) = 0#

    glEnableClientState GL_COLOR_ARRAY
    glEnableClientState GL_VERTEX_ARRAY

    glColorPointer 3, GL_DOUBLE, 0, VarPtr(Color(0))
    glVertexPointer 3, GL_DOUBLE, 0, VarPtr(Vertex(0))

    ' IMPORTANT: count is number of vertices (3), NOT number of doubles (9)
