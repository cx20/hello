...
const D3D12_RESOURCE_STATE_GENERIC_READ  = 0xAC3;
...
const D3D12_ROOT_PARAMETER_TYPE_CBV = 2;
...
const VTBL_COMMAND_LIST_SET_ROOT_CBV = 38;
...
$d3d12Cdef = <<<CDEF
    ...
    typedef void (__stdcall *SetGraphicsRootConstantBufferViewFunc)(void* pThis, UINT RootParameterIndex, UINT64 BufferLocation);
...
$windowName = wstr("Raymarching (PHP DirectX12)");
...
$d3dCompile = FFI::cast($d3d12Types->type('D3DCompileFunc'), $pfnD3DCompile);
    ...
    // Root signature
    $rootParam = $d3d12Types->new('D3D12_ROOT_PARAMETER');
    $rootParam->ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
    $rootParam->u->Descriptor->ShaderRegister = 0;
    $rootParam->u->Descriptor->RegisterSpace = 0;
    $rootParam->ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;

    $rsDesc = $d3d12Types->new('D3D12_ROOT_SIGNATURE_DESC');
    FFI::memset(FFI::addr($rsDesc), 0, FFI::sizeof($rsDesc));
    $rsDesc->NumParameters = 1;
    $rsDesc->pParameters = FFI::addr($rootParam);
    ...
    $inputLayout = $d3d12Types->new('D3D12_INPUT_ELEMENT_DESC[1]');
    ...
    $verts[0]->x = -1.0; $verts[0]->y = -1.0;
    $verts[1]->x = -1.0; $verts[1]->y = 3.0;
    ...
    if ($hr < 0) {
        echo "CreateCommittedResource(CB) failed (hr=0x" . dechex($hr & 0xffffffff) . ")\n";
        exit(1);
    }
    $gCbvBuffer = FFI::cast($d3d12Types->type('IUnknown*'), out_ptr($d3d12Types, $ppCb));

    $cbMap = FFI::cast($d3d12Types->type('MapResourceFunc'), $gCbvBuffer->lpVtbl[VTBL_RESOURCE_MAP]);
    $cbMapped = FFI::new('void*');
    $cbReadRange = $d3d12Types->new('D3D12_RANGE');
    $cbReadRange->Begin = 0;
    $cbReadRange->End = 0;
    $hr = $cbMap($gCbvBuffer, 0, FFI::addr($cbReadRange), FFI::addr($cbMapped));
    if ($hr < 0) {
        echo "Map(CB) failed (hr=0x" . dechex($hr & 0xffffffff) . ")\n";
        exit(1);
    }
    $gCbvData = $cbMapped;

    $getCbGpuVA = FFI::cast($d3d12Types->type('GetGPUVirtualAddressFunc'), $gCbvBuffer->lpVtbl[VTBL_RESOURCE_GET_GPU_VA]);
    $gCbvGpuVA = $getCbGpuVA($gCbvBuffer);

    echo "[PHPD3D12] constant buffer created\n";
    ...
    while ($user32->IsWindow($hwnd)) {
        ...
        if (isset($gCbvGpuVA) && $gCbvData !== null) {
            $t = microtime(true) - $startTime;
            $cbVals = $d3d12Types->new('float[4]');
            $cbVals[0] = (float)$t;
            $cbVals[1] = (float)$width;
            $cbVals[2] = (float)$height;
            $cbVals[3] = 0.0;
            FFI::memcpy($gCbvData, FFI::addr($cbVals[0]), FFI::sizeof($cbVals));

            $setRootCbv = FFI::cast($d3d12Types->type('SetGraphicsRootConstantBufferViewFunc'), $gCommandList->lpVtbl[VTBL_COMMAND_LIST_SET_ROOT_CBV]);
            $setRootCbv($gCommandList, 0, $gCbvGpuVA);
        }
    ...
    if (isset($gCbvBuffer)) {
        com_release($d3d12Types, $gCbvBuffer);
    }
