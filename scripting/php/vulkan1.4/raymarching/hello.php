...
/*
  ...
  Raymarching features:
  - Animated 3D scene with spheres, boxes, and torus
...
$windowName = wbuf("Vulkan 1.4 Raymarching (PHP)");
...
$vulkan_dev = FFI::cdef('
    ...
    void vkCmdPushConstants(VkCommandBuffer commandBuffer, VkPipelineLayout layout, uint32_t stageFlags, uint32_t offset, uint32_t size, const void* pValues);
...
// Push constants for time and resolution
$pushConstantRange = $vulkan_dev->new('VkPushConstantRange[1]');
$pushConstantRange[0]->stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
$pushConstantRange[0]->offset = 0;
$pushConstantRange[0]->size = 16; // float iTime + float padding + vec2 iResolution = 16 bytes
...
$pipelineLayoutCreateInfo[0]->pPushConstantRanges = FFI::addr($pushConstantRange[0]);
...
// Record command buffers (will be done dynamically in render loop)
echo "[INIT] Command buffers will be recorded dynamically\n";
...
while (!$bQuit) {
    ...
    if ($user32->PeekMessageW(FFI::addr($msg), null, 0, 0, PM_REMOVE) != 0) {
        ...
        // Dynamically record command buffer with push constants
        $iTime = (float)(microtime(true) - $start_time);
        
        // Reset command buffer
        $vulkan_dev->vkResetCommandBuffer($commandBuffers[$imgIdx], 0);
        ...
        $commandBufferBeginInfo[0]->flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        $commandBufferBeginInfo[0]->pInheritanceInfo = null;

        $vulkan_dev->vkBeginCommandBuffer($commandBuffers[$imgIdx], FFI::addr($commandBufferBeginInfo[0]));
        ...
        $renderPassBeginInfo[0]->framebuffer = $framebuffers[$imgIdx];
        ...
        $vulkan_dev->vkCmdBeginRenderPass($commandBuffers[$imgIdx], FFI::addr($renderPassBeginInfo[0]), 0);
        $vulkan_dev->vkCmdBindPipeline($commandBuffers[$imgIdx], VK_PIPELINE_BIND_POINT_GRAPHICS, $graphicsPipeline[0]);

        // Push constants: iTime (float), padding (float), iResolution (vec2)
        $pushData = FFI::new('float[4]');
        $pushData[0] = $iTime;
        $pushData[1] = 0.0; // padding
        $pushData[2] = (float)$extent->width;
        $pushData[3] = (float)$extent->height;
        $vulkan_dev->vkCmdPushConstants($commandBuffers[$imgIdx], $pipelineLayout[0], VK_SHADER_STAGE_FRAGMENT_BIT, 0, 16, FFI::addr($pushData[0]));

        $vulkan_dev->vkCmdSetViewport($commandBuffers[$imgIdx], 0, 1, FFI::addr($viewport[0]));
        $vulkan_dev->vkCmdSetScissor($commandBuffers[$imgIdx], 0, 1, FFI::addr($scissor[0]));

        $vulkan_dev->vkCmdDraw($commandBuffers[$imgIdx], 3, 1, 0, 0);

        $vulkan_dev->vkCmdEndRenderPass($commandBuffers[$imgIdx]);
        $vulkan_dev->vkEndCommandBuffer($commandBuffers[$imgIdx]);
