#!/usr/bin/env ruby
# -*- coding: utf-8 -*-
# Vulkan Raymarching (Windows, Ruby, Fiddle)
#
# Raymarching demo with push constants:
# - Animated 3D scene with SDF rendering (spheres, boxes, torus)
# - Push constants for time and resolution
# - Dynamic command buffer recording per frame
# - Phong lighting with soft shadows and ambient occlusion
...
def vk_pipeline_layout_ci
  # VkPushConstantRange: stageFlags(4) + offset(4) + size(4) + padding(4) = 16 bytes
  push_range_data = ''
  push_range_data += [VK_SHADER_STAGE_FRAGMENT_BIT].pack('L')  # stageFlags
  push_range_data += [0].pack('L')                              # offset
  push_range_data += [16].pack('L')                             # size
  push_range_data += [0].pack('L')                              # padding for alignment
  p_push_range = blob(push_range_data)
  
  # VkPipelineLayoutCreateInfo (48 bytes on x64)
  # sType(4) + pad(4) + pNext(8) + flags(4) + setLayoutCount(4) + pSetLayouts(8) + 
  # pushConstantRangeCount(4) + pad(4) + pPushConstantRanges(8)
  plci_data = ''
  plci_data += [VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO].pack('L')
  plci_data += [0].pack('L')                # padding
  plci_data += [0].pack('Q')                # pNext
  plci_data += [0].pack('L')                # flags
  plci_data += [0].pack('L')                # setLayoutCount
  plci_data += [0].pack('Q')                # pSetLayouts
  plci_data += [1].pack('L')                # pushConstantRangeCount
  plci_data += [0].pack('L')                # padding
  plci_data += [p_push_range].pack('Q')     # pPushConstantRanges
...
vkCmdPushConstants = vk_dev_fn(vkGetDeviceProcAddr, device, 'vkCmdPushConstants', Fiddle::TYPE_VOID, [Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP, Fiddle::TYPE_UINT, Fiddle::TYPE_UINT, Fiddle::TYPE_UINT, Fiddle::TYPE_VOIDP])
...
# Command buffers will be recorded dynamically in render loop
log_msg("Command buffers allocated (will record per-frame for push constants)")
...
while pump_messages
  ...
  # Record command buffer dynamically with push constants
  elapsed = Time.now - start_time
  cb = cmd_buffers[idx]
  
  vk_check(vkResetCommandBuffer.call(cb, 0), "vkResetCommandBuffer")
  ...
  vkCmdBeginRenderPass.call(cb, p_rpbi, 0)
  vkCmdBindPipeline.call(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline)
  
  # Push constants: iTime, padding, iResolution.x, iResolution.y
  push_data = [elapsed.to_f, 0.0, cur_w.to_f, cur_h.to_f].pack('f4')
  vkCmdPushConstants.call(cb, pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 16, blob(push_data))
