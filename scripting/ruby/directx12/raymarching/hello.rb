...
# ============================================================
# HLSL (load from hello.hlsl)
# ============================================================
hlsl_path = File.join(__dir__, 'hello.hlsl')
HLSL_SRC = File.read(hlsl_path)
...
def init_d3d
  ...
  # D3D12_ROOT_PARAMETER (x64):
  # UINT ParameterType (4)
  # union (16): for ROOT_DESCRIPTOR => ShaderRegister (4), RegisterSpace (4), pad (8)
  # UINT ShaderVisibility (4)
  # pad (4)
  root_param_data = [
    DX12::D3D12_ROOT_PARAMETER_TYPE_CBV, # ParameterType
    0,                                   # ShaderRegister
    0                                    # RegisterSpace
  ].pack('L3') + "\0" * 8 + [DX12::D3D12_SHADER_VISIBILITY_ALL].pack('L') + "\0" * 4
  root_param = Mem.to_ptr(root_param_data)

  rs_desc = [1].pack('L') + "\0" * 4 +
            [root_param.to_i].pack('Q') +
            [0].pack('L') + "\0" * 4 +
            [0].pack('Q') +
            [DX12::D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT].pack('L') +
            "\0" * 4
  rs_desc = Mem.to_ptr(rs_desc)
  log "RootSignature desc ptr=0x%016X" % rs_desc.to_i
  log "RootSignature param ptr=0x%016X" % root_param.to_i
  log "RootSignature param bytes=#{Fiddle::Pointer.new(root_param.to_i)[0, 24].unpack('L6').inspect}"
  rs_blob_pp = Mem.malloc(8)
  rs_err_pp  = Mem.malloc(8)
  hr = DX12.D3D12SerializeRootSignature(rs_desc, 1, rs_blob_pp, rs_err_pp)
  log "D3D12SerializeRootSignature hr=#{hr_hex(hr)}"
  if hr != 0
    err = rs_err_pp[0, 8].unpack1('Q')
    if err != 0
      p = COM.blob_ptr(err)
      s = COM.blob_size(err)
      msg = Fiddle::Pointer.new(p)[0, s]
      log "RootSignature error blob size=#{s}"
      log "RootSignature error: #{msg}"
      COM.release(err)
    end
    raise "D3D12SerializeRootSignature failed (hr=#{hr_hex(hr)})"
  end

  rs_blob = rs_blob_pp[0, 8].unpack1('Q')
  rs_ptr  = COM.blob_ptr(rs_blob)
  rs_size = COM.blob_size(rs_blob)
  log "RootSignature blob size=#{rs_size} ptr=0x%016X" % rs_ptr
  ...
  COM.release(rs_blob)
  log "CreateRootSignature hr=#{hr_hex(hr)}"
  ...
  # InputLayout @552
  semantic = Mem.to_ptr("POSITION\0")
  input_elem = Mem.to_ptr([
    semantic.to_i,
    0,
    DX12::DXGI_FORMAT_R32G32_FLOAT,
    0,
    0,
    0,
    0
  ].pack('QLLLLLL'))
  input_layout = [input_elem.to_i, 1].pack('QL') + "\0" * 4
  CRT.memcpy(pso_desc + 552, Mem.to_ptr(input_layout), input_layout.bytesize)
  ...
  # Vertex buffer (fullscreen triangle) + constant buffer
  log "Create vertex buffer"
  heap_props = Mem.to_ptr([DX12::D3D12_HEAP_TYPE_UPLOAD, 0, 0, 1, 1].pack('LLLLL') + "\0" * 4)
  vb_data = [-1.0, -1.0, -1.0, 3.0, 3.0, -1.0].pack('f6')
  vb_size = vb_data.bytesize

  res_desc = [
    DX12::D3D12_RESOURCE_DIMENSION_BUFFER,
    0,
    vb_size,
    1,
    1,
    ...
    1,
    0,
    DX12::D3D12_TEXTURE_LAYOUT_ROW_MAJOR,
    DX12::D3D12_RESOURCE_FLAG_NONE
  ].pack('L x4 Q Q L S S L L L L x4')

  vb_pp = Mem.malloc(8)
  hr = COM.call($device, 27, Fiddle::TYPE_INT,
                [Fiddle::TYPE_VOIDP, Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP],
                heap_props, 0, Mem.to_ptr(res_desc), DX12::D3D12_RESOURCE_STATE_GENERIC_READ, 0, IID_ID3D12Resource, vb_pp)
  log "CreateCommittedResource(vertex) hr=#{hr_hex(hr)}"
  hr_check(hr, "CreateCommittedResource(vertex)")
  $vertex_buffer = vb_pp[0, 8].unpack1('Q')
  log "Vertex buffer=0x%016X" % $vertex_buffer

  # Map and upload vertex data
  mapped_pp = Mem.malloc(8)
  range = Mem.to_ptr([0, 0].pack('QQ'))
  hr = COM.call($vertex_buffer, 8, Fiddle::TYPE_INT,
                [Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP],
                0, range, mapped_pp)
  log "Map(vertex) hr=#{hr_hex(hr)}"
  hr_check(hr, "Map(vertex)")
  mapped = mapped_pp[0, 8].unpack1('Q')
  log "Vertex mapped ptr=0x%016X" % mapped
  Fiddle::Pointer.new(mapped)[0, vb_size] = vb_data
  COM.call($vertex_buffer, 9, Fiddle::TYPE_VOID, [Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP], 0, 0)

  gpu_va = COM.call($vertex_buffer, 11, Fiddle::TYPE_LONG_LONG, [])
  log "Vertex GPU VA=0x%016X" % gpu_va
  $vb_view = Mem.to_ptr([gpu_va, vb_size, 8].pack('QLL'))

  # Constant buffer (256 bytes)
  cb_size = 256
  cb_desc = [
    DX12::D3D12_RESOURCE_DIMENSION_BUFFER,
    0,
  ...
  hr = COM.call($device, 27, Fiddle::TYPE_INT,
                [Fiddle::TYPE_VOIDP, Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP],
                heap_props, 0, Mem.to_ptr(cb_desc), DX12::D3D12_RESOURCE_STATE_GENERIC_READ, 0, IID_ID3D12Resource, cb_pp)
  log "CreateCommittedResource(constant) hr=#{hr_hex(hr)}"
  hr_check(hr, "CreateCommittedResource(constant)")
  $cbuffer = cb_pp[0, 8].unpack1('Q')
  log "Constant buffer=0x%016X" % $cbuffer

  cb_mapped_pp = Mem.malloc(8)
  hr = COM.call($cbuffer, 8, Fiddle::TYPE_INT,
                [Fiddle::TYPE_INT, Fiddle::TYPE_VOIDP, Fiddle::TYPE_VOIDP],
                0, range, cb_mapped_pp)
  log "Map(constant) hr=#{hr_hex(hr)}"
  hr_check(hr, "Map(constant)")
  $cbuffer_cpu = cb_mapped_pp[0, 8].unpack1('Q')
  log "Constant mapped ptr=0x%016X" % $cbuffer_cpu
  $cbuffer_gpu = COM.call($cbuffer, 11, Fiddle::TYPE_LONG_LONG, [])
  log "Constant GPU VA=0x%016X" % $cbuffer_gpu
...
def render
  ...
  if $cbuffer_cpu != 0 && $cbuffer_gpu != 0
    t = Time.now.to_f - $start_time
    cb = [t, $width.to_f, $height.to_f, 0.0].pack('f4')
    Fiddle::Pointer.new($cbuffer_cpu)[0, cb.bytesize] = cb
    COM.call($command_list, 38, Fiddle::TYPE_VOID,
             [Fiddle::TYPE_INT, Fiddle::TYPE_LONG_LONG], 0, $cbuffer_gpu)
  end
  ...
  # IASetVertexBuffers (index 44)
  if $vb_view
    COM.call($command_list, 44, Fiddle::TYPE_VOID,
...
def cleanup
  ...
  COM.release($vertex_buffer) if $vertex_buffer != 0
  COM.release($cbuffer) if $cbuffer != 0
...
puts "=== DirectX 12 Raymarching (Ruby x64) ==="
Win32.CoInitialize(0)

hInstance = Win32.GetModuleHandleW(0)
cls_name  = Mem.to_ptr(wstr_z("RubyDX12"))
win_title = Mem.to_ptr(wstr_z("DirectX 12 Raymarching (Ruby)"))
