...
' ============================================================
'  Excel VBA (64-bit) + Raw OpenGL 3.3 Core Profile
'   - Creates a Win32 window (CS_OWNDC)
'   - Creates temporary legacy context, loads wglCreateContextAttribsARB
'   - Creates OpenGL 3.3 Core Profile context
'   - Renders triangle via VAO + VBO + glVertexAttribPointer + GLSL 330
'   - Debug log: C:\TEMP\debug.log
'
'  NOTE:
'   - Core Profile requires VAO.
'   - We use small x64 thunks to call some GL entry points safely.
' ============================================================
...
' Class / window names
' -----------------------------
Private Const CLASS_NAME As String = "helloWindowVBA_GL33"
Private Const WINDOW_NAME As String = "Hello OpenGL 3.3 Core (VBA64)"
...
Private Function BuildThunk_Gen2(ByVal target As LongPtr) As LongPtr
    ' x64 code (same pattern as previous):
    ' sub rsp,28h
    ' mov rcx, rdx
    ' mov rdx, r8
    ' mov rax, imm64
...
'   Thunk loads args from [wParam] and calls target with correct ABI.
' ============================================================
Private Function BuildThunk_VAP(ByVal target As LongPtr) As LongPtr
    ' x64 code:
    ' sub rsp,38h              ; shadow + 2 stack args, keep 16B alignment
    ' mov r10, r8              ; r10 = args*
    ' mov rcx, [r10+0]         ; index
    ' mov rdx, [r10+8]         ; size
    ' mov r8,  [r10+16]        ; type
...
'   - choose pixel format (legacy)
'   - create temp legacy context
'   - load wglCreateContextAttribsARB
'   - create core 3.3 context, replace
' ============================================================
Private Function CreateGL33CoreContext(ByVal hWnd As LongPtr, ByVal hDC As LongPtr) As LongPtr
    LogMsg "CreateGL33CoreContext: start"
    ...
    ' attrib list for 3.3 core
    Dim attribs(0 To 8) As Long
    attribs(0) = WGL_CONTEXT_MAJOR_VERSION_ARB: attribs(1) = 3
    attribs(2) = WGL_CONTEXT_MINOR_VERSION_ARB: attribs(3) = 3
    ...
    LogMsg "CreateGL33CoreContext: creating core 3.3..."
    ' CallWindowProcW maps: rcx=hDC, rdx=share(0), r8=attribPtr, r9=0
    ...
    If hRC_new = 0 Then
        Err.Raise vbObjectError + 8005, , "wglCreateContextAttribsARB failed (core 3.3)"
    End If
    ...
    LogMsg "CreateGL33CoreContext: done"
    CreateGL33CoreContext = hRC_new
End Function
...
' ============================================================
' Load OpenGL 3.3 core functions + build thunks
' ============================================================
Private Sub LoadGL33Functions()
    LogMsg "LoadGL33Functions: start"
    ...
    ' build thunks (Gen2 and VAP)
    If p_thunkGen2 = 0 Then
        LogMsg "BuildThunk_Gen2..."
        ' One thunk can be used for both glGenBuffers and glGenVertexArrays IF we embed target.
        ' So we will build per-target and reuse by rebuilding as needed (simpler: keep one for buffers and call it, then rebuild for VAO).
        ' Here: build for glGenBuffers, then later rebuild for glGenVertexArrays.
    ...
    LogMsg "LoadGL33Functions: done"
End Sub

' ============================================================
' Create shader program (GLSL 330 core)
' ============================================================
Private Sub CreateProgram()
    LogMsg "CreateProgram: start"

    Dim vs As Long, fs As Long
    vs = CompileShader(GL_VERTEX_SHADER, VertSrc(), "VS330")
    fs = CompileShader(GL_FRAGMENT_SHADER, FragSrc(), "FS330")
    ...
    ' Optional: delete shader objects after link
...
Private Sub InitGeometry()
    ...
    ' Rebuild Gen2 thunk for glGenVertexArrays (embed different target)
    LogMsg "InitGeometry: rebuild Gen2 thunk for glGenVertexArrays"
    If p_thunkGen2 <> 0 Then VirtualFree p_thunkGen2, 0, MEM_RELEASE: p_thunkGen2 = 0
...
Public Sub Main()
    ...
    g_hRC = CreateGL33CoreContext(g_hWnd, g_hDC)
    LogMsg "GL33 core hRC=" & Hex$(g_hRC)
