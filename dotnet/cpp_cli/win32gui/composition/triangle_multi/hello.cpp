...
//   cl /EHa /std:c++17 /clr /I%VULKAN_SDK%\Include hello_dcomp_multi.cpp ^
...
static void InitDirectComposition() {
    ...
    IDXGIDevice* dxgiDev = nullptr;
    g_d3dDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&dxgiDev);
    DCompositionCreateDevice(dxgiDev, __uuidof(IDCompositionDevice), (void**)&g_dcompDevice);
    dxgiDev->Release();
...
static void InitOpenGL() {
    ...
    if (p_glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        throw std::runtime_error("GL FBO not complete");
    p_glBindFramebuffer(GL_FRAMEBUFFER, 0);

    // Bind SwapChain to DComp visual
    g_glVisual->SetContent(g_glSwapChain);
...
// ************************************************************
//  PANEL 2: Vulkan (offscreen Å® staging Å® D3D11 copy)
// ************************************************************
...
static void InitVulkan() {
    ...
    g_vkVisual->SetContent(g_vkSwapChain);
    dbg("[VK] ok\n");
}
...
static void RenderVulkan() {
    ...
    // Copy Vulkan staging Å® D3D11 staging Å® back buffer
    void* vkData = nullptr;
...
// ************************************************************
//  Commit and main loop
// ************************************************************
static void CommitAndRun() {
    // Commit the visual tree to DWM
    g_dcompDevice->Commit();
    dbg("[Main] DComp Commit ok - entering message loop\n");
