...
Public Module Hello
    ...
    Private Const DXGI_FLIP_SEQ As UInteger = 3UI
    Private Const DXGI_ALPHA_PRE As UInteger = 1UI
    ...
    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCCommitDelegate(d As IntPtr) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCCreateTargetDelegate(d As IntPtr, hw As IntPtr, top As Integer, ByRef t As IntPtr) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCCreateVisDelegate(d As IntPtr, ByRef v As IntPtr) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCSetRootDelegate(t As IntPtr, v As IntPtr) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCSetOffXDelegate(v As IntPtr, x As Single) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCSetOffYDelegate(v As IntPtr, y As Single) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCSetContentDelegate(v As IntPtr, c As IntPtr) As Integer

    <UnmanagedFunctionPointer(CallingConvention.StdCall)>
    Private Delegate Function DCAddVisDelegate(v As IntPtr, ch As IntPtr, above As Integer, rf As IntPtr) As Integer
    ...
    <DllImport("dcomp.dll")>
    Private Function DCompositionCreateDevice(dxgi As IntPtr, ByRef iid As Guid, ByRef dcomp As IntPtr) As Integer
    End Function
    ...
    Private ReadOnly IID_DCDev As New Guid("C37EA93A-E7AA-450D-B16F-9746CB0407F3")

    ' Writes timestamped messages to DebugView.
    Private Sub Dbg(msg As String)
        Try
            OutputDebugString("[VB.DComp.Multi] " & DateTime.Now.ToString("HH:mm:ss.fff") & " " & msg)
        Catch
    ...
    Private _dc As IntPtr
    Private _dct As IntPtr
    Private _dcr As IntPtr
    ...
    Private Sub MakeWin()
        Dbg("MakeWin: begin")
        Const WINDOW_TITLE As String = "Hello, DirectComposition(VB.NET) World!"
    ...
    Private Sub MakeDC()
        Dbg("MakeDC: begin")
        Dim dxgi As IntPtr = IntPtr.Zero
        Dim dg As Guid = IID_DXGIDev
        CheckHR(QI(_d3d, dg, dxgi), "QI IDXGIDevice")

        Dim dcg As Guid = IID_DCDev
        CheckHR(DCompositionCreateDevice(dxgi, dcg, _dc), "DCompositionCreateDevice")
        Rel(dxgi)

        CType(Marshal.GetDelegateForFunctionPointer(VT(_dc, 6), GetType(DCCreateTargetDelegate)), DCCreateTargetDelegate)(_dc, _hw, 1, _dct)
        CType(Marshal.GetDelegateForFunctionPointer(VT(_dc, 7), GetType(DCCreateVisDelegate)), DCCreateVisDelegate)(_dc, _dcr)
        CType(Marshal.GetDelegateForFunctionPointer(VT(_dct, 3), GetType(DCSetRootDelegate)), DCSetRootDelegate)(_dct, _dcr)
        Dbg("MakeDC: dcomp=0x" & _dc.ToInt64().ToString("X"))
    End Sub

    Private Function DCVis() As IntPtr
        Dim v As IntPtr = IntPtr.Zero
        CType(Marshal.GetDelegateForFunctionPointer(VT(_dc, 7), GetType(DCCreateVisDelegate)), DCCreateVisDelegate)(_dc, v)
        Return v
    End Function

    Private Sub DCSetup(v As IntPtr, sc As IntPtr, x As Single)
        CType(Marshal.GetDelegateForFunctionPointer(VT(v, 15), GetType(DCSetContentDelegate)), DCSetContentDelegate)(v, sc)
        CType(Marshal.GetDelegateForFunctionPointer(VT(v, 4), GetType(DCSetOffXDelegate)), DCSetOffXDelegate)(v, x)
        CType(Marshal.GetDelegateForFunctionPointer(VT(v, 6), GetType(DCSetOffYDelegate)), DCSetOffYDelegate)(v, 0.0F)
        CType(Marshal.GetDelegateForFunctionPointer(VT(_dcr, 16), GetType(DCAddVisDelegate)), DCAddVisDelegate)(_dcr, v, 1, IntPtr.Zero)
    End Sub

    Private Sub DCCommit()
        CType(Marshal.GetDelegateForFunctionPointer(VT(_dc, 3), GetType(DCCommitDelegate)), DCCommitDelegate)(_dc)
    End Sub
    ...
    Private Function MakeSC(w As Integer, h As Integer) As IntPtr
        Dim d As New DXGI_SWAP_CHAIN_DESC1 With {
            ...
            .BufferCount = 2UI,
            .SwapEffect = DXGI_FLIP_SEQ,
            .AlphaMode = DXGI_ALPHA_PRE
        }
    ...
    Public Sub Main()
        Try
            Dbg("=== START ===")
            Console.WriteLine("=== GL + D3D11 + VK via DirectComposition (VB.NET) ===")
