...
type DispatcherQueueOptions =
    ...
    val mutable threadType: int
    val mutable apartmentType: int
...
[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type CreateDesktopWindowTargetDelegate = delegate of nativeint * nativeint * int * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type CreateCompSurfaceForSCDelegate = delegate of nativeint * nativeint * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type CreateContainerVisualDelegate = delegate of nativeint * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type CreateSpriteVisualDelegate = delegate of nativeint * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type CreateSurfaceBrushWithSurfaceDelegate = delegate of nativeint * nativeint * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type PutRootDelegate = delegate of nativeint * nativeint -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type GetChildrenDelegate = delegate of nativeint * nativeint byref -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type InsertAtTopDelegate = delegate of nativeint * nativeint -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type PutBrushDelegate = delegate of nativeint * nativeint -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type PutSizeDelegate = delegate of nativeint * Float2 -> int

[<UnmanagedFunctionPointer(CallingConvention.StdCall)>]
type PutOffsetDelegate = delegate of nativeint * Float3 -> int
...
module NativeMethods =
    ...
    extern int CreateDispatcherQueueController([<In; Out>] DispatcherQueueOptions& options, [<Out>] nativeint& controller)

    [<DllImport("combase.dll", PreserveSig = true)>]
    extern int RoInitialize(int initType)

    [<DllImport("combase.dll", PreserveSig = true)>]
    extern int RoActivateInstance(nativeint activatableClassId, [<Out>] nativeint& instance)

    [<DllImport("combase.dll", PreserveSig = true)>]
    extern int WindowsCreateString([<MarshalAs(UnmanagedType.LPWStr)>] string sourceString, uint32 length, [<Out>] nativeint& hstring)
...
module App =
    ...
    let DXGI_FLIP_DISCARD = 4u
    let DXGI_ALPHA_IGNORE = 0u
    let D3D_SDK = 7u
    ...
    let IID_ICompositorDesktopInterop = Guid("29E691FA-4567-4DCA-B319-D0F207EB6807")
    let IID_ICompositorInterop = Guid("25297D5C-3AD4-4C9C-B5CF-E36A38512330")
    let IID_ICompositor = Guid("B403CA50-7F8C-4E83-985F-CC45060036D8")
    let IID_ICompositionTarget = Guid("A1BEA8BA-D726-4663-8129-6B5E7927FFA6")
    let IID_IContainerVisual = Guid("02F6BC74-ED20-4773-AFE6-D49B4A93DB32")
    let IID_IVisualCollection = Guid("8B745505-FD3E-4A98-84A8-E949468C6BCB")
    let IID_ISpriteVisual = Guid("08E05581-1AD1-4F97-9757-402D76E4233B")
    let IID_IVisual = Guid("117E202D-A859-4C89-873B-C2AA566788E3")
    let IID_ICompositionBrush = Guid("AB0D7608-30C0-40E9-B568-B60A6BD1FB46")
    ...
    let mutable dq = nativeint 0
    let mutable compositorUnk = nativeint 0
    let mutable compositor = nativeint 0
    let mutable desktopTarget = nativeint 0
    let mutable compTarget = nativeint 0
    let mutable rootContainer = nativeint 0
    let mutable rootVisual = nativeint 0
    let mutable children = nativeint 0
    let spriteRaw = [| nativeint 0; nativeint 0; nativeint 0 |]
    let spriteVisual = [| nativeint 0; nativeint 0; nativeint 0 |]
    let brush = [| nativeint 0; nativeint 0; nativeint 0 |]
    ...
    let makeWin () =
        ...
        hw <- NativeMethods.CreateWindowEx(0u, "DCMulti", "OpenGL + D3D11 + Vulkan via Windows.UI.Composition (F#)", WS_OVR ||| WS_VIS, 100, 100, rc.Right - rc.Left, rc.Bottom - rc.Top, nativeint 0, nativeint 0, hi, nativeint 0)
    ...
    let addCompositionPanel (sc: nativeint) (offsetX: float32) (index: int) =
        let compInterop = qi compositorUnk IID_ICompositorInterop
        let mutable surface = nativeint 0
        let createSurface = Marshal.GetDelegateForFunctionPointer<CreateCompSurfaceForSCDelegate>(vt compInterop 4)
        checkHR "CreateCompositionSurfaceForSwapChain" (createSurface.Invoke(compInterop, sc, &surface))
        rel compInterop

        let mutable surfBrush = nativeint 0
        let createBrush = Marshal.GetDelegateForFunctionPointer<CreateSurfaceBrushWithSurfaceDelegate>(vt compositor 24)
        checkHR "CreateSurfaceBrush" (createBrush.Invoke(compositor, surface, &surfBrush))
        rel surface
        brush.[index] <- qi surfBrush IID_ICompositionBrush
        rel surfBrush

        let mutable raw = nativeint 0
        let createSprite = Marshal.GetDelegateForFunctionPointer<CreateSpriteVisualDelegate>(vt compositor 22)
        checkHR "CreateSpriteVisual" (createSprite.Invoke(compositor, &raw))
        spriteRaw.[index] <- raw

        let putBrush = Marshal.GetDelegateForFunctionPointer<PutBrushDelegate>(vt raw 7)
        checkHR "put_Brush" (putBrush.Invoke(raw, brush.[index]))

        let visual = qi raw IID_IVisual
        spriteVisual.[index] <- visual

        let putSize = Marshal.GetDelegateForFunctionPointer<PutSizeDelegate>(vt visual 36)
        let putOffset = Marshal.GetDelegateForFunctionPointer<PutOffsetDelegate>(vt visual 21)
        checkHR "put_Size" (putSize.Invoke(visual, Float2(X = float32 PW, Y = float32 PH)))
        checkHR "put_Offset" (putOffset.Invoke(visual, Float3(X = offsetX, Y = 0.0f, Z = 0.0f)))

        let insert = Marshal.GetDelegateForFunctionPointer<InsertAtTopDelegate>(vt children 9)
        checkHR "InsertAtTop" (insert.Invoke(children, visual))

    let initComposition () =
        dbg "initComposition: begin"
        let hrRo = NativeMethods.RoInitialize(0)
        if hrRo < 0 && hrRo <> 1 then checkHR "RoInitialize" hrRo

        let mutable dqOpt = DispatcherQueueOptions(dwSize = Marshal.SizeOf(typeof<DispatcherQueueOptions>), threadType = 2, apartmentType = 0)
        let hrDQ = NativeMethods.CreateDispatcherQueueController(&dqOpt, &dq)
        checkHR "CreateDispatcherQueueController" hrDQ

        let className = "Windows.UI.Composition.Compositor"
        let mutable hstr = nativeint 0
        checkHR "WindowsCreateString" (NativeMethods.WindowsCreateString(className, uint32 className.Length, &hstr))
        try
            checkHR "RoActivateInstance" (NativeMethods.RoActivateInstance(hstr, &compositorUnk))
        finally
            NativeMethods.WindowsDeleteString(hstr) |> ignore

        compositor <- qi compositorUnk IID_ICompositor

        let deskInterop = qi compositorUnk IID_ICompositorDesktopInterop
        let createTarget = Marshal.GetDelegateForFunctionPointer<CreateDesktopWindowTargetDelegate>(vt deskInterop 3)
        let mutable dt = nativeint 0
        let hrTarget = createTarget.Invoke(deskInterop, hw, 0, &dt)
        rel deskInterop
        checkHR "CreateDesktopWindowTarget" hrTarget
        desktopTarget <- dt

        compTarget <- qi desktopTarget IID_ICompositionTarget
        let createContainer = Marshal.GetDelegateForFunctionPointer<CreateContainerVisualDelegate>(vt compositor 9)
        let mutable rc = nativeint 0
        checkHR "CreateContainerVisual" (createContainer.Invoke(compositor, &rc))
        rootContainer <- rc

        rootVisual <- qi rootContainer IID_IVisual
        let putRoot = Marshal.GetDelegateForFunctionPointer<PutRootDelegate>(vt compTarget 7)
        checkHR "put_Root" (putRoot.Invoke(compTarget, rootVisual))

        let getChildren = Marshal.GetDelegateForFunctionPointer<GetChildrenDelegate>(vt rootContainer 6)
        let mutable ch = nativeint 0
        checkHR "get_Children" (getChildren.Invoke(rootContainer, &ch))
        children <- ch

        addCompositionPanel glSC 0.0f 0
        addCompositionPanel dxSC (float32 PW) 1
        addCompositionPanel vSC (float32 (PW * 2)) 2
    ...
    let makeSC (w: int) (h: int) : nativeint =
        ...
        d.SwapEffect <- DXGI_FLIP_DISCARD
        d.AlphaMode <- DXGI_ALPHA_IGNORE
        d.Flags <- 0u
    ...
    let main _ =
        ...
        try
            dbg "=== START ==="
            printfn "=== GL + D3D11 + VK via Windows.UI.Composition (F#) ==="
            makeWin()
            makeD3D()
            makeFac()
            printfn "--- GL ---"
