...
module OpenGL =
    ...
    type wglCreateContextAttribsARBDelegate = delegate of IntPtr * IntPtr * int[] -> IntPtr
...
type HelloForm() as this =
    ...
    member private this.EnableOpenGL() =
        ...
        let hGLRC_old = OpenGL.wglCreateContext(hDC)
        OpenGL.wglMakeCurrent(hDC, hGLRC_old) |> ignore

        let wglCreateContextAttribsARBPtr = OpenGL.wglGetProcAddress("wglCreateContextAttribsARB")
        let wglCreateContextAttribsARB = Marshal.GetDelegateForFunctionPointer<OpenGL.wglCreateContextAttribsARBDelegate>(wglCreateContextAttribsARBPtr)

        hGLRC <- wglCreateContextAttribsARB.Invoke(hDC, IntPtr.Zero, null)
        OpenGL.wglMakeCurrent(hDC, hGLRC) |> ignore
        OpenGL.wglDeleteContext(hGLRC_old) |> ignore
