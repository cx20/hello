...
//
// Strategy: JScript.NET cannot express unsafe code, StructLayout attributes, or
// UnmanagedFunctionPointer delegates natively, so the implementation is compiled
// at runtime from embedded C# source using CSharpCodeProvider.
...
// Build a one-shot dynamic P/Invoke type and call MessageBoxW.
// This is the same technique as InvokeWin32() used in other JScript.NET samples
// in this repo, applied directly to MessageBoxW so error dialogs are visible
// even when the process is compiled as /target:winexe (no console window).
// OutputDebugString P/Invoke — output visible in Sysinternals DebugView.
// Uses the same dynamic-assembly technique as other JScript.NET samples in this repo.
...
function ShowMsgBox(text:String, title:String):void {
  try {
    var domain  = AppDomain.CurrentDomain;
    var aname   = new AssemblyName("_MBAsm");
    var asmB    = domain.DefineDynamicAssembly(aname, AssemblyBuilderAccess.Run);
    var modB    = asmB.DefineDynamicModule("_MBMod");
    var typeB   = modB.DefineType("_MBType",
                    TypeAttributes.Public | TypeAttributes.BeforeFieldInit);
    var methB   = typeB.DefineMethod("MessageBoxW",
                    MethodAttributes.Public  | MethodAttributes.HideBySig |
                    MethodAttributes.Static  | MethodAttributes.PinvokeImpl,
    ...
    var ctor    = DllImportAttribute.GetConstructor([Type.GetType("System.String")]);
    methB.SetCustomAttribute(new CustomAttributeBuilder(ctor, ["user32.dll"]));
    typeB.CreateType().InvokeMember("MessageBoxW",
      BindingFlags.Public | BindingFlags.Static | BindingFlags.InvokeMethod,
      null, null, [IntPtr.Zero, text, title, 0x10]);
  } catch(ignored) {}
}

function RunOnSTA():void {
  Dbg("RunOnSTA: start");
  var provider = new CSharpCodeProvider();
  var parameters = new CompilerParameters();
  parameters.GenerateInMemory = true;
  parameters.GenerateExecutable = false;
  // Do NOT pass /platform:x64 — let the compiler match the host-process bitness.
  parameters.CompilerOptions = "/unsafe";
  parameters.ReferencedAssemblies.Add("System.dll");

  Dbg("RunOnSTA: compiling embedded C#...");
  var source  = csharpLines.join("\n");
  var results = provider.CompileAssemblyFromSource(parameters, source);

  if (results.Errors.HasErrors) {
    Dbg("RunOnSTA: compile FAILED (" + results.Errors.Count + " errors)");
    var sb:String = "C# compilation failed:\n";
    for (var i = 0; i < results.Errors.Count; i++) {
      var err = results.Errors[i];
      var line = "  [" + err.Line + "," + err.Column + "] " + err.ErrorNumber + ": " + err.ErrorText;
      Dbg(line);
      sb += line + "\n";
    }
    ShowMsgBox(sb, "JScript.NET - Compile Error");
    return;
  }

  Dbg("RunOnSTA: compile OK — invoking Hello.Main()");
  try {
    var asm  = results.CompiledAssembly;
    var type = asm.GetType("Hello");
    // Main() has no access modifier inside the class → private; use NonPublic flag.
    var main = type.GetMethod("Main",
                 BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
    if (main == null) {
      Dbg("RunOnSTA: Hello.Main not found!");
      ShowMsgBox("Hello.Main() not found via reflection.", "JScript.NET - Error");
      return;
    }
    main.Invoke(null, null);
    Dbg("RunOnSTA: Hello.Main() returned normally");
  } catch (e) {
    Dbg("RunOnSTA: runtime exception: " + e.ToString());
    ShowMsgBox("Runtime error:\n" + e.ToString(), "JScript.NET - Runtime Error");
  }
}

function Main() {
  // DirectComposition works on both MTA and STA; call the implementation
  // directly on the JScript.NET main thread.
  RunOnSTA();
}

// ---------------------------------------------------------------------------
// Embedded C# source — identical logic to Hello.cs with the window title
// changed to reflect JScript.NET as the host language.
// ---------------------------------------------------------------------------
var csharpLines = [
  "using System;",
  "using System.IO;",
  ...
  "                string e = Marshal.PtrToStringAnsi(shaderc_result_get_error_message(r));",
  "                shaderc_result_release(r);",
  "                throw new Exception(\"Shader: \" + e);",
  "            }",
  "            int len = (int)(ulong)shaderc_result_get_length(r);",
  "            byte[] d = new byte[len];",
  ...
  "",
  "    [DllImport(\"user32.dll\", CharSet=CharSet.Unicode)] static extern ushort RegisterClassEx(ref WNDCLASSEX w);",
  "    [DllImport(\"user32.dll\", CharSet=CharSet.Unicode)] static extern IntPtr CreateWindowEx(uint ex, string cls, string ttl, uint st, int x, int y, int w, int h, IntPtr p, IntPtr m, IntPtr hi, IntPtr lp);",
  ...
  "    [DllImport(\"kernel32.dll\")] static extern void OutputDebugStringA([MarshalAs(UnmanagedType.LPStr)] string s);",
  "    static void Dbg(string s) { OutputDebugStringA(\"[Hello.cs] \" + s); }",
  "",
  ...
  "        _hw = CreateWindowEx(0, \"DCMulti\", \"Hello, DirectComposition(JScript.NET) World!\", WS_OVR | WS_VIS, 100, 100, rc.Right - rc.Left, rc.Bottom - rc.Top, IntPtr.Zero, IntPtr.Zero, hi, IntPtr.Zero);",
  "        Dbg(\"MakeWin: _hw=\" + _hw.ToString(\"X\"));",
  "    }",
  "    static void MakeD3D() {",
  "        Dbg(\"MakeD3D: start\");",
  ...
  "        Dbg(\"MakeD3D: OK hr=\" + hr.ToString(\"X8\"));",
  "    }",
  "    static void MakeFac() {",
  "        Dbg(\"MakeFac: start\");",
  "        Guid g = new Guid(\"770aae78-f26f-4dba-a829-253c83d1b387\");",
  "        IntPtr f1; CreateDXGIFactory1(ref g, out f1); QI(f1, IID_F2, out _fac); Rel(f1);",
  "        Dbg(\"MakeFac: OK\");",
  "    }",
  "    static void MakeDC() {",
  "        Dbg(\"MakeDC: start\");",
  "        Guid dg = IID_DXGIDev;",
  "        IntPtr dxgi; QI(_d3d, dg, out dxgi);",
  "        Dbg(\"MakeDC: DCompositionCreateDevice\");",
  "        Guid dcg = IID_DCDev; int hdc=DCompositionCreateDevice(dxgi, ref dcg, out _dc); Rel(dxgi);",
  "        Dbg(\"MakeDC: DCompositionCreateDevice hr=\" + hdc.ToString(\"X8\"));",
  ...
  "        Dbg(\"MakeDC: OK\");",
  "    }",
  "    static IntPtr DCVis() { IntPtr v; Marshal.GetDelegateForFunctionPointer<DCCreateVisDelegate>(VT(_dc, 7))(_dc, out v); return v; }",
  ...
  "        IntPtr sc; Marshal.GetDelegateForFunctionPointer<CreateSCCompDelegate>(VT(_fac, 24))(_fac, _d3d, ref d, IntPtr.Zero, out sc); return sc;",
  "    }",
  "    static IntPtr SCBuf(IntPtr sc) { Guid g = IID_Tex2D; IntPtr t; Marshal.GetDelegateForFunctionPointer<GetBufDelegate>(VT(sc, 9))(sc, 0, ref g, out t); return t; }",
  "    static void SCPres(IntPtr sc) { Marshal.GetDelegateForFunctionPointer<PresentDelegate>(VT(sc, 8))(sc, 1, 0); }",
  "    static IntPtr MakeRTV(IntPtr t) { IntPtr r; Marshal.GetDelegateForFunctionPointer<CreateRTVDelegate>(VT(_d3d, 9))(_d3d, t, IntPtr.Zero, out r); return r; }",
  ...
  "        IntPtr t; Marshal.GetDelegateForFunctionPointer<CreateTex2DDelegate>(VT(_d3d, 5))(_d3d, ref d, IntPtr.Zero, out t); return t;",
  "    }",
  ...
  "    static D3D11_MAPPED_SUBRESOURCE MapW(IntPtr r) { D3D11_MAPPED_SUBRESOURCE m; Marshal.GetDelegateForFunctionPointer<MapDelegate>(VT(_ctx, 14))(_ctx, r, 0, D3D_MAP_W, 0, out m); return m; }",
  ...
  "        IntPtr vb, pb, _dxErr;",
  "        D3DCompile(HLSL, (IntPtr)HLSL.Length, \"dx\", IntPtr.Zero, IntPtr.Zero, \"VS\", \"vs_4_0\", 0, 0, out vb, out _dxErr);",
  "        D3DCompile(HLSL, (IntPtr)HLSL.Length, \"dx\", IntPtr.Zero, IntPtr.Zero, \"PS\", \"ps_4_0\", 0, 0, out pb, out _dxErr);",
  ...
  "        DCSetup(DCVis(), _dxSC, PW); Dbg(\"InitDX: OK\");",
  "    }",
  ...
  "        DCSetup(DCVis(), _glSC, 0); Dbg(\"InitGL: OK\");",
  "    }",
  ...
  "    }",
  "",
  "    // VK panel — uses hello.vert / hello.frag (GLSL) compiled via shaderc",
  ...
  "        vkGetDeviceQueue(_vD,(uint)_vQF,0,out _vQ); VkPhysMemProps mp; vkGetPhysicalDeviceMemoryProperties(_vPD,out mp);",
  ...
  "        VkMemReq ir; vkGetImageMemoryRequirements(_vD,_vOI,out ir); var ia2=new VkMemAI{sType=5,size=ir.size,memIdx=FindMT(ref mp,ir.memBits,1)}; vkAllocateMemory(_vD,ref ia2,IntPtr.Zero,out _vOM); vkBindImageMemory(_vD,_vOI,_vOM,0);",
  ...
  "        VkMemReq br; vkGetBufferMemoryRequirements(_vD,_vSB,out br); var ba=new VkMemAI{sType=5,size=br.size,memIdx=FindMT(ref mp,br.memBits,6)}; vkAllocateMemory(_vD,ref ba,IntPtr.Zero,out _vSM2); vkBindBufferMemory(_vD,_vSB,_vSM2,0);",
  ...
  "        // Compile GLSL shaders from hello.vert / hello.frag using shaderc",
  ...
  "        DCSetup(DCVis(), _vSC, PW*2); Dbg(\"InitVK: OK\");",
  "    }",
  ...
  "        IntPtr vd; vkMapMemory(_vD,_vSM2,0,(ulong)(PW*PH*4),0,out vd);",
  ...
  "        Dbg(\"Main: start\");",
  "        Dbg(\"Main: MakeWin\"); MakeWin();",
  "        Dbg(\"Main: MakeD3D\"); MakeD3D();",
  "        Dbg(\"Main: MakeFac\"); MakeFac();",
  "        Dbg(\"Main: MakeDC\");  MakeDC();",
  "        Dbg(\"Main: InitGL\");  InitGL();",
  "        Dbg(\"Main: InitDX\");  InitDX();",
  "        Dbg(\"Main: InitVK\");  InitVK();",
  "        Dbg(\"Main: DCCommit\"); DCCommit();",
  ...
  "            if(first){Dbg(\"Main: first frame OK\");first=false;} System.Threading.Thread.Sleep(1);",
  "        }",
  "        vkDeviceWaitIdle(_vD); Dbg(\"Main: END\");",
  "    }",
  "}"
];
