...
def init_wgl_and_gl_funcs():
    ...
    global glGetUniformLocation, glUniform1f, glUniform2f
...
time_location = ctypes.c_int(-1)
resolution_location = ctypes.c_int(-1)
start_time = 0.0
...
def init_raymarching_resources():
    global program, time_location, resolution_location, start_time
    ...
    time_location = ctypes.c_int(glGetUniformLocation(program.value, b"iTime"))
    resolution_location = ctypes.c_int(glGetUniformLocation(program.value, b"iResolution"))
...
def draw():
    ...
    glUniform1f(time_location.value, current_time)
    glUniform2f(resolution_location.value, float(g_width), float(g_height))
...
def wndproc(hwnd, msg, wparam, lparam):
    ...
    if msg == WM_SIZE:
        ...
        if g_width > 0 and g_height > 0:
            opengl32.glViewport(0, 0, g_width, g_height)
        return 0
...
# ---------------------------------------
def main():
    global g_hdc, g_hrc, g_hwnd, g_width, g_height

    hInstance = kernel32.GetModuleHandleW(None)
    className = "PyOpenGL46Raymarching"
    ...
    g_hwnd = user32.CreateWindowExW(
        0, className, "Raymarching - OpenGL 4.6 (ctypes only)",
        WS_OVERLAPPEDWINDOW,
    ...
    rect = RECT()
    user32.GetClientRect(g_hwnd, ctypes.byref(rect))
    g_width = rect.right - rect.left
    g_height = rect.bottom - rect.top
    opengl32.glViewport(0, 0, g_width, g_height)

    init_raymarching_resources()
    ...
    destroy_context(g_hwnd, g_hdc, g_hrc)
