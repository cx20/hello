...
// Linux (X11) + Manual Vulkan Bindings
// Compile: gdc -o hello hello.d -lX11 -ldl
...
extern(C) {
    ...
    struct XSetWindowAttributes {
        ulong background_pixmap, background_pixel, border_pixmap, border_pixel;
        int bit_gravity, win_gravity, backing_store;
        ulong backing_planes, backing_pixel;
        Bool_X save_under;
        long event_mask, do_not_propagate_mask;
        Bool_X override_redirect;
        ulong colormap, cursor;
    }

    struct XClientMessageEvent {
        int type; ulong serial; Bool_X send_event;
        Display* display; Window window; Atom message_type; int format;
        union _Data { byte[20] b; short[10] s; long[5] l; }
        _Data data;
    }
    ...
    int      XSetStandardProperties(Display*, Window, const(char)*, const(char)*, Pixmap, char**, int, void*);
    Atom     XInternAtom(Display*, const(char)*, Bool_X);
    Status   XSetWMProtocols(Display*, Window, Atom*, int);
    int      XMapRaised(Display*, Window);
    int      XPending(Display*);
    int      XNextEvent(Display*, XEvent*);
    int      XDestroyWindow(Display*, Window);
    void     XSync(Display*, Bool_X);
}
...
extern(C) {
    ...
    alias PFN_vkCreateXlibSurfaceKHR = VkResult function(VkInstance, const(VkXlibSurfaceCreateInfoKHR)*, const(void)*, VkSurfaceKHR*);
...
bool loadVulkan() {
    gVulkanLib = dlopen("libvulkan.so.1", RTLD_NOW | RTLD_LOCAL);
    if (gVulkanLib is null) {
        gVulkanLib = dlopen("libvulkan.so", RTLD_NOW | RTLD_LOCAL);
        if (gVulkanLib is null) {
            printf("Failed to load libvulkan.so.1\n");
            return false;
        }
    }
...
void createSurface() {
    ...
    createInfo.dpy    = gApp.display;
    createInfo.window = gApp.window;

    if (vkCreateXlibSurfaceKHR(gApp.instance, &createInfo, null, &gApp.surface) != VK_SUCCESS) {
...
void drawFrame() {
    ...
    if (result == VK_ERROR_SURFACE_LOST_KHR) {
        printf("ERROR_SURFACE_LOST_KHR: Vulkan WSI is not supported in this environment.\n");
        printf("A hardware GPU is required. WSL software rendering (lavapipe) does not support Vulkan surface operations.\n");
        exit(1);
    }
...
void cleanup() {
    ...
    XDestroyWindow(gApp.display, gApp.window);
    XCloseDisplay(gApp.display);
}
...
void initWindow() {
    gApp.display  = XOpenDisplay(null);
    if (gApp.display is null) {
        printf("Failed to open X display\n");
        exit(1);
    }
    gApp.screenId = XDefaultScreen(gApp.display);

    XSetWindowAttributes attribs;
    attribs.border_pixel     = XBlackPixel(gApp.display, gApp.screenId);
    attribs.background_pixel = XBlackPixel(gApp.display, gApp.screenId);
    attribs.event_mask       = ExposureMask | ButtonPressMask | KeyPressMask;

    gApp.window = XCreateWindow(
        gApp.display, XDefaultRootWindow(gApp.display),
        0, 0, WIDTH, HEIGHT,
        0, 0, InputOutput, null,
        CWBorderPixel | CWBackPixel | CWEventMask, &attribs);

    XSetStandardProperties(gApp.display, gApp.window, "Hello, World!", null, 0, null, 0, null);

    gApp.atomWmDeleteWindow = XInternAtom(gApp.display, "WM_DELETE_WINDOW", 0);
    XSetWMProtocols(gApp.display, gApp.window, &gApp.atomWmDeleteWindow, 1);

    XMapRaised(gApp.display, gApp.window);
    XSync(gApp.display, 0);
}
...
void mainLoop() {
    bool done = false;
    while (!done) {
        while (XPending(gApp.display) > 0) {
            XEvent ev;
            XNextEvent(gApp.display, &ev);
            if (ev.type == ClientMessage) {
                if (ev.xclient.data.l[0] == cast(long)gApp.atomWmDeleteWindow)
                    done = true;
