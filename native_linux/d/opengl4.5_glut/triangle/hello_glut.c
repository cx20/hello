...
static void Initialize(void)
{
    ...
    glEnableVertexAttribArray(0); glVertexAttribPointer(0,3,GL_FLOAT,GL_FALSE,0,(const void*)0);
    glBindBuffer(GL_ARRAY_BUFFER,gBufs[1]); glBufferData(GL_ARRAY_BUFFER,sizeof(cols),cols,GL_STATIC_DRAW);
    glEnableVertexAttribArray(1); glVertexAttribPointer(1,3,GL_FLOAT,GL_FALSE,0,(const void*)0);
    glBindVertexArray(0);
}
...
static void Display(void)
{
    ...
    glBindVertexArray(gVAO); glDrawArrays(GL_TRIANGLES,0,3); glBindVertexArray(0);
    glutSwapBuffers();
}
...
static void OnClose(void){ glDeleteVertexArrays(1,&gVAO); glDeleteBuffers(2,gBufs); glDeleteProgram(gProgram); glDeleteShader(gFS); glDeleteShader(gVS); exit(0); }
...
void runSample(void)
{
    ...
    glutCreateWindow("Hello, OpenGL 4.5 World!");
    Initialize();
