...
public class HarmonographForm : Form
{
    // OpenGL constants
...
";
    ...
    public HarmonographForm()
    {
        ...
        this.Text = "OpenGL 4.6 Compute Harmonograph (PowerShell)";
    ...
    void OnTimer(object sender, EventArgs e)
    {
        ...
        if ((DateTime.Now - lastFpsTime).TotalMilliseconds >= 1000)
        {
            ...
            this.Text = String.Format("OpenGL 4.6 Compute Harmonograph (PowerShell) - FPS: {0:F1}", fps);
        }
    }
    ...
    T GetDelegate<T>(string name) where T : class
    {
    ...
    uint CompileShader(uint type, string source)
    {
        ...
        if (status != GL_TRUE)
        {
            ...
            if (logLen > 1)
            {
                System.Text.StringBuilder log = new System.Text.StringBuilder(logLen);
                int actualLen;
