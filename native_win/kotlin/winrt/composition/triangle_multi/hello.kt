...
// ============================================================
// Kotlin/Native  EWindows.UI.Composition composite of three triangles:
...
//   d3dcompiler_47.dll, combase.dll, coremessaging.dll
// ============================================================
...
private val IID_ICompositor = heapGuid(
    0xB403CA50u, 0x7F8Cu, 0x4E83u,
    0x98.toByte(), 0x5F.toByte(), 0xCC.toByte(), 0x45.toByte(),
    0x06.toByte(), 0x00.toByte(), 0x36.toByte(), 0xD8.toByte()
)
private val IID_ICompositorDesktopInterop = heapGuid(
    0x29E691FAu, 0x4567u, 0x4DCAu,
    0xB3.toByte(), 0x19.toByte(), 0xD0.toByte(), 0xF2.toByte(),
    0x07.toByte(), 0xEB.toByte(), 0x68.toByte(), 0x07.toByte()
)
private val IID_ICompositorInterop = heapGuid(
    0x25297D5Cu, 0x3AD4u, 0x4C9Cu,
    0xB5.toByte(), 0xCF.toByte(), 0xE3.toByte(), 0x6A.toByte(),
    0x38.toByte(), 0x51.toByte(), 0x23.toByte(), 0x30.toByte()
)
private val IID_ICompositionTarget = heapGuid(
    0xA1BEA8BAu, 0xD726u, 0x4663u,
    0x81.toByte(), 0x29.toByte(), 0x6B.toByte(), 0x5E.toByte(),
    0x79.toByte(), 0x27.toByte(), 0xFF.toByte(), 0xA6.toByte()
)
private val IID_IVisual = heapGuid(
    0x117E202Du, 0xA859u, 0x4C89u,
    0x87.toByte(), 0x3B.toByte(), 0xC2.toByte(), 0xAA.toByte(),
    0x56.toByte(), 0x67.toByte(), 0x88.toByte(), 0xE3.toByte()
)
private val IID_ICompositionBrush = heapGuid(
    0xAB0D7608u, 0x30C0u, 0x40E9u,
    0xB5.toByte(), 0x68.toByte(), 0xB6.toByte(), 0x0A.toByte(),
    0x6B.toByte(), 0xD1.toByte(), 0xFB.toByte(), 0x46.toByte()
)
...
// WinRT init constants
private const val RO_INIT_SINGLETHREADED = 0u
private const val DQTYPE_THREAD_CURRENT = 2u
private const val DQTAT_COM_NONE = 0u
private const val HSTRING_HEADER_SIZE = 40

// Windows.UI.Composition vtable slots
private const val WUC_COMPOSITOR_CREATE_CONTAINER_VISUAL = 9
private const val WUC_COMPOSITOR_CREATE_SPRITE_VISUAL = 22
private const val WUC_COMPOSITOR_CREATE_SURFACE_BRUSH = 24
private const val WUC_DESKTOP_INTEROP_CREATE_TARGET = 3
private const val WUC_INTEROP_CREATE_SURFACE_FOR_SC = 4
private const val WUC_TARGET_PUT_ROOT = 7
private const val WUC_CONTAINER_GET_CHILDREN = 6
private const val WUC_VISUALCOLLECTION_INSERT_TOP = 9
private const val WUC_SPRITE_PUT_BRUSH = 7
private const val WUC_VISUAL_PUT_OFFSET = 21
private const val WUC_VISUAL_PUT_SIZE = 36
...
private fun MemScope.createHStringRef(
    s: String,
    windowsCreateStringReference: CPointer<CFunction<(COpaquePointer?, UInt, COpaquePointer?, CPointer<COpaquePointerVar>) -> Int>>
): COpaquePointer? {
    val hs = alloc<COpaquePointerVar>()
    val header = allocArray<ByteVar>(HSTRING_HEADER_SIZE)
    val hr = windowsCreateStringReference(s.wcstr.ptr, s.length.toUInt(), header, hs.ptr)
    if (hr < 0) throw RuntimeException("WindowsCreateStringReference failed: 0x${hr.toUInt().toString(16)}")
    return hs.value
}

private fun packVec2(x: Float, y: Float): Long {
    val xb = x.toBits().toLong() and 0xFFFFFFFFL
    val yb = y.toBits().toLong() and 0xFFFFFFFFL
    return xb or (yb shl 32)
}
...
// ============================================================
fun main() {
    debugPrint("=== OpenGL + D3D11 + Vulkan via Windows.UI.Composition (Kotlin/Native) ===")
    ...
    val combaseDll = LoadLibraryW("combase.dll") ?: throw RuntimeException("Cannot load combase.dll")
    val coreMessagingDll = LoadLibraryW("coremessaging.dll") ?: throw RuntimeException("Cannot load coremessaging.dll")
    ...
    // ---- Load D3D11 / DXGI / WinRT entry points ----
    ...
    val pfnRoInitialize: CPointer<CFunction<(UInt) -> Int>> = loadDll(combaseDll, "RoInitialize")
    val pfnRoActivateInstance: CPointer<CFunction<(COpaquePointer?, CPointer<COpaquePointerVar>) -> Int>> =
        loadDll(combaseDll, "RoActivateInstance")
    val pfnWindowsCreateStringReference: CPointer<CFunction<(COpaquePointer?, UInt, COpaquePointer?, CPointer<COpaquePointerVar>) -> Int>> =
        loadDll(combaseDll, "WindowsCreateStringReference")
    val pfnCreateDispatcherQueueController: CPointer<CFunction<(COpaquePointer?, CPointer<COpaquePointerVar>) -> Int>> =
        loadDll(coreMessagingDll, "CreateDispatcherQueueController")
    ...
    memScoped {
        ...
        // Windows.UI.Composition path: use default extended style.
        val hWnd = CreateWindowExW(
            0u, CLASS_NAME,
            "OpenGL + D3D11 + Vulkan via Windows.UI.Composition (Kotlin/Native)",
            style.toUInt(),
        ...
        // ============================================================
        // 3. Windows.UI.Composition setup
        // ============================================================
        hr = pfnRoInitialize(RO_INIT_SINGLETHREADED)
        if (hr < 0) throw RuntimeException("RoInitialize failed: 0x${hr.toUInt().toString(16)}")

        val dqControllerOut = alloc<COpaquePointerVar>()
        val dqOpts = zalloc(12) // DispatcherQueueOptions
        dqOpts.w32(0, 12u)
        dqOpts.w32(4, DQTYPE_THREAD_CURRENT)
        dqOpts.w32(8, DQTAT_COM_NONE)
        hr = pfnCreateDispatcherQueueController(dqOpts, dqControllerOut.ptr)
        if (hr < 0) throw RuntimeException("CreateDispatcherQueueController failed: 0x${hr.toUInt().toString(16)}")
        val dqController = dqControllerOut.value

        val compositorUnkOut = alloc<COpaquePointerVar>()
        val compositorClass = createHStringRef("Windows.UI.Composition.Compositor", pfnWindowsCreateStringReference)
        hr = pfnRoActivateInstance(compositorClass, compositorUnkOut.ptr)
        if (hr < 0) throw RuntimeException("RoActivateInstance(Compositor) failed: 0x${hr.toUInt().toString(16)}")
        val compositorUnk = compositorUnkOut.value!!

        val compositorOut = alloc<COpaquePointerVar>()
        hr = comQI(compositorUnk, IID_ICompositor, compositorOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("QI(ICompositor) failed: 0x${hr.toUInt().toString(16)}")
        val compositor = compositorOut.value!!

        val desktopInteropOut = alloc<COpaquePointerVar>()
        hr = comQI(compositorUnk, IID_ICompositorDesktopInterop, desktopInteropOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("QI(ICompositorDesktopInterop) failed: 0x${hr.toUInt().toString(16)}")
        val desktopInterop = desktopInteropOut.value!!

        val compInteropOut = alloc<COpaquePointerVar>()
        hr = comQI(compositorUnk, IID_ICompositorInterop, compInteropOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("QI(ICompositorInterop) failed: 0x${hr.toUInt().toString(16)}")
        val compInterop = compInteropOut.value!!

        val desktopTargetOut = alloc<COpaquePointerVar>()
        val fnCreateTarget = slot(desktopInterop, WUC_DESKTOP_INTEROP_CREATE_TARGET)!!
            .reinterpret<CFunction<(COpaquePointer?, HWND?, Int, COpaquePointer?) -> Int>>()
        hr = fnCreateTarget(desktopInterop, hWnd, 1, desktopTargetOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("CreateDesktopWindowTarget failed: 0x${hr.toUInt().toString(16)}")
        val desktopTarget = desktopTargetOut.value!!

        val compTargetOut = alloc<COpaquePointerVar>()
        hr = comQI(desktopTarget, IID_ICompositionTarget, compTargetOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("QI(ICompositionTarget) failed: 0x${hr.toUInt().toString(16)}")
        val compTarget = compTargetOut.value!!

        val rootContainerOut = alloc<COpaquePointerVar>()
        val fnCreateContainer = slot(compositor, WUC_COMPOSITOR_CREATE_CONTAINER_VISUAL)!!
            .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
        hr = fnCreateContainer(compositor, rootContainerOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("CreateContainerVisual failed: 0x${hr.toUInt().toString(16)}")
        val rootContainer = rootContainerOut.value!!

        val rootVisualOut = alloc<COpaquePointerVar>()
        hr = comQI(rootContainer, IID_IVisual, rootVisualOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("QI(IVisual root) failed: 0x${hr.toUInt().toString(16)}")
        val rootVisual = rootVisualOut.value!!

        val fnPutRoot = slot(compTarget, WUC_TARGET_PUT_ROOT)!!
            .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
        hr = fnPutRoot(compTarget, rootVisual)
        if (hr < 0) throw RuntimeException("ICompositionTarget.put_Root failed: 0x${hr.toUInt().toString(16)}")

        val visualCollectionOut = alloc<COpaquePointerVar>()
        val fnGetChildren = slot(rootContainer, WUC_CONTAINER_GET_CHILDREN)!!
            .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
        hr = fnGetChildren(rootContainer, visualCollectionOut.ptr.reinterpret())
        if (hr < 0) throw RuntimeException("IContainerVisual.get_Children failed: 0x${hr.toUInt().toString(16)}")
        val visualCollection = visualCollectionOut.value!!

        fun addSwapChainVisual(sc: COpaquePointer, offsetX: Float): COpaquePointer {
            val surfaceOut = alloc<COpaquePointerVar>()
            val fnCreateSurfaceForSc = slot(compInterop, WUC_INTEROP_CREATE_SURFACE_FOR_SC)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?, COpaquePointer?) -> Int>>()
            hr = fnCreateSurfaceForSc(compInterop, sc, surfaceOut.ptr.reinterpret())
            if (hr < 0) throw RuntimeException("CreateCompositionSurfaceForSwapChain failed: 0x${hr.toUInt().toString(16)}")
            val surface = surfaceOut.value!!

            val brushOut = alloc<COpaquePointerVar>()
            val fnCreateSurfaceBrush = slot(compositor, WUC_COMPOSITOR_CREATE_SURFACE_BRUSH)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?, COpaquePointer?) -> Int>>()
            hr = fnCreateSurfaceBrush(compositor, surface, brushOut.ptr.reinterpret())
            if (hr < 0) throw RuntimeException("CreateSurfaceBrush failed: 0x${hr.toUInt().toString(16)}")
            val brush = brushOut.value!!

            val compBrushOut = alloc<COpaquePointerVar>()
            hr = comQI(brush, IID_ICompositionBrush, compBrushOut.ptr.reinterpret())
            if (hr < 0) throw RuntimeException("QI(ICompositionBrush) failed: 0x${hr.toUInt().toString(16)}")
            val compBrush = compBrushOut.value!!

            val spriteOut = alloc<COpaquePointerVar>()
            val fnCreateSprite = slot(compositor, WUC_COMPOSITOR_CREATE_SPRITE_VISUAL)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
            hr = fnCreateSprite(compositor, spriteOut.ptr.reinterpret())
            if (hr < 0) throw RuntimeException("CreateSpriteVisual failed: 0x${hr.toUInt().toString(16)}")
            val sprite = spriteOut.value!!

            val fnPutBrush = slot(sprite, WUC_SPRITE_PUT_BRUSH)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
            hr = fnPutBrush(sprite, compBrush)
            if (hr < 0) throw RuntimeException("ISpriteVisual.put_Brush failed: 0x${hr.toUInt().toString(16)}")

            val visOut = alloc<COpaquePointerVar>()
            hr = comQI(sprite, IID_IVisual, visOut.ptr.reinterpret())
            if (hr < 0) throw RuntimeException("QI(IVisual from sprite) failed: 0x${hr.toUInt().toString(16)}")
            val vis = visOut.value!!

            val fnPutSize = slot(vis, WUC_VISUAL_PUT_SIZE)!!
                .reinterpret<CFunction<(COpaquePointer?, Long) -> Int>>()
            hr = fnPutSize(vis, packVec2(PANEL_W.toFloat(), PANEL_H.toFloat()))
            if (hr < 0) throw RuntimeException("IVisual.put_Size failed: 0x${hr.toUInt().toString(16)}")

            val fnPutOffset = slot(vis, WUC_VISUAL_PUT_OFFSET)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
            val off = zalloc(12)
            off.wF(0, offsetX); off.wF(4, 0.0f); off.wF(8, 0.0f)
            hr = fnPutOffset(vis, off)
            if (hr < 0) throw RuntimeException("IVisual.put_Offset failed: 0x${hr.toUInt().toString(16)}")

            val fnInsertTop = slot(visualCollection, WUC_VISUALCOLLECTION_INSERT_TOP)!!
                .reinterpret<CFunction<(COpaquePointer?, COpaquePointer?) -> Int>>()
            hr = fnInsertTop(visualCollection, vis)
            if (hr < 0) throw RuntimeException("IVisualCollection.InsertAtTop failed: 0x${hr.toUInt().toString(16)}")

            comRelease(surface)
            comRelease(compBrush)
            comRelease(brush)
            comRelease(sprite)
            return vis
        }
        ...
        val vkVisual = addSwapChainVisual(vkSwapChain, (PANEL_W * 2).toFloat())
        debugPrint("[INIT] Windows.UI.Composition visual tree built")
        ...
        // ============================================================
        // Windows.UI.Composition visual tree is now active
        // ============================================================
        debugPrint("[INIT] Windows.UI.Composition activated")
        ...
        // Windows.UI.Composition cleanup
        comRelease(vkVisual)
        comRelease(dxVisual)
        comRelease(glVisual)
        comRelease(visualCollection)
        comRelease(rootContainer)
        comRelease(rootVisual)
        comRelease(compTarget)
        comRelease(desktopTarget)
        comRelease(compInterop)
        comRelease(desktopInterop)
        comRelease(compositor)
        comRelease(compositorUnk)
        comRelease(dqController)
