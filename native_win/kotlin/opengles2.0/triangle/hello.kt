...
// Send log lines to console and DebugView.
...
private const val WINDOW_NAME = "Hello OpenGL ES 2.0 (Kotlin/Native)"
...
// WGL constants for OpenGL ES context creation.
...
private const val WGL_CONTEXT_ES2_PROFILE_BIT_EXT: Int = 0x00000004
...
// OpenGL ES 2.0 shader sources (same style as C sample).
...
// Create an OpenGL ES 2.0 context via WGL.
private fun enableOpenGL(hdc: HDC?): HGLRC? = memScoped {
    debugPrint("[enableOpenGL] Starting OpenGL ES 2.0 context creation...")
    ...
    val createCtxPtr = wglGetProcAddress("wglCreateContextAttribsARB")
    if (createCtxPtr == null) {
        debugPrint("[enableOpenGL] wglCreateContextAttribsARB unavailable, using legacy context")
        return@memScoped hrcOld
    }

    val wglCreateContextAttribsARB: CPointer<PFN_wglCreateContextAttribsARB> = createCtxPtr.reinterpret()
    ...
    attribs[7] = WGL_CONTEXT_ES2_PROFILE_BIT_EXT
    attribs[8] = 0

    val hrc = wglCreateContextAttribsARB(hdc, null, attribs)
    if (hrc == null) {
        debugPrint("[enableOpenGL] ES2 context creation failed (GetLastError=${GetLastError()}), using legacy context")
    ...
    if (wglMakeCurrent(hdc, hrc) == 0) {
        debugPrint("[enableOpenGL] ERROR: wglMakeCurrent (ES2) failed")
        wglDeleteContext(hrc)
        return@memScoped null
    }

    debugPrint("[enableOpenGL] OpenGL ES 2.0 context is active")
    hrc
}
...
private fun loadGLFunctions() {
    fun <T : CFunction<*>> load(name: String): CPointer<T> {
        ...
        if (p1 != null) return p1.reinterpret()
...
private fun checkShaderCompilation(shader: UInt, shaderType: String) = memScoped {
    ...
    if (status.value != 1) {
        ...
        if (logLength.value > 0) {
            ...
            debugPrint("[checkShaderCompilation] $shaderType log: ${logBuffer.toKString()}")
        }
    }
}
...
private fun checkProgramLink(program: UInt) = memScoped {
    ...
    if (status.value != 1) {
        ...
        if (logLength.value > 0) {
            ...
            debugPrint("[checkProgramLink] Program log: ${logBuffer.toKString()}")
        }
    }
}
...
private fun initShaderAndBuffers() = memScoped {
    ...
    if (posAttrib >= 0) glEnableVertexAttribArray(posAttrib.toUInt())

    colAttrib = glGetAttribLocation(shaderProgram, "color".cstr.ptr)
    if (colAttrib >= 0) glEnableVertexAttribArray(colAttrib.toUInt())
}
...
fun main() {
    initDebugOutput()
    debugPrint("[main] Starting Kotlin/Native OpenGL ES 2.0 triangle sample")
    ...
    memScoped {
        ...
        SetWindowTextW(hWnd, "Hello OpenGL ES 2.0 (Kotlin/Native) - GL $glVersion")
