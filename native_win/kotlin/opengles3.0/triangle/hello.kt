...
private const val WINDOW_NAME = "Hello OpenGL ES 3.0 (Kotlin/Native)"
...
// OpenGL ES 3.0 shader sources (same style as C sample).
...
// Create an OpenGL ES 3.0 context via WGL.
private fun enableOpenGL(hdc: HDC?): HGLRC? = memScoped {
    debugPrint("[enableOpenGL] Starting OpenGL ES 3.0 context creation...")
    ...
    if (hrc == null) {
        debugPrint("[enableOpenGL] ES3 context creation failed (GetLastError=${GetLastError()}), using legacy context")
    ...
    if (wglMakeCurrent(hdc, hrc) == 0) {
        debugPrint("[enableOpenGL] ERROR: wglMakeCurrent (ES3) failed")
        wglDeleteContext(hrc)
        return@memScoped null
    }

    debugPrint("[enableOpenGL] OpenGL ES 3.0 context is active")
    hrc
}
...
fun main() {
    initDebugOutput()
    debugPrint("[main] Starting Kotlin/Native OpenGL ES 3.0 triangle sample")
    ...
    memScoped {
        ...
        SetWindowTextW(hWnd, "Hello OpenGL ES 3.0 (Kotlin/Native) - GL $glVersion")
