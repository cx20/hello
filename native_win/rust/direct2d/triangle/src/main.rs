...
impl Window {
    ...
    fn draw(&self, target: &ID2D1DeviceContext) -> Result<()> {
        unsafe {
            ...
            let p1 = D2D_POINT_2F { x: (width * 1.0 / 2.0), y: (height * 1.0 / 4.0) };
            let p2 = D2D_POINT_2F { x: (width * 3.0 / 4.0), y: (height * 3.0 / 4.0) };
            let p3 = D2D_POINT_2F { x: (width * 1.0 / 4.0), y: (height * 3.0 / 4.0) };

            let brush = self.brush.as_ref().unwrap();

            target.DrawLine(p1, p2, brush, 1.0, &self.style);
            target.DrawLine(p2, p3, brush, 1.0, &self.style);
            target.DrawLine(p3, p1, brush, 1.0, &self.style);
        ...
        unsafe {
            ...
            loop {
                if self.visible {
                    ...
                    let _ = GetMessageA(&mut message, None, 0, 0);
