...
(*
  Vulkan 1.4 Triangle (Windows, Free Pascal, no external Pascal libs)

  - Win32 window (Windows unit)
  - Vulkan loaded dynamically from vulkan-1.dll
  - Swapchain + render pass + graphics pipeline
  - Draw triangle using gl_VertexIndex (no vertex buffer)
  - Shaders are compiled at runtime from hello.vert / hello.frag using shaderc_shared.dll

  Build:
    build.bat  (requires Vulkan SDK for shaderc_shared.dll + fpc)
*)
...
begin
  ...
  if b.pipelineLayout <> 0 then vkDestroyPipelineLayout(device, b.pipelineLayout, nil);
...
procedure RecordCommandBuffers(device: VkDevice; var b: TSwapchainBundle);
var
  i: Integer;
...
procedure CreateSwapchainBundle(instance: VkInstance; device: VkDevice; pd: VkPhysicalDevice; surface: VkSurfaceKHR;
  hwnd: HWND; graphicsQ, presentQ: UInt32; const vertGlsl, fragGlsl: string; out b: TSwapchainBundle);
var
  ...
  rpci: TVkRenderPassCreateInfo;
	vertWords, fragWords: TUInt32DynArray;
...
begin
  ...
  // Shaders (runtime compile: GLSL -> SPIR-V)
  vertWords := CompileGLSLFileToWords(vertGlsl, shaderc_glsl_vertex_shader);
  fragWords := CompileGLSLFileToWords(fragGlsl, shaderc_glsl_fragment_shader);

  vertMod := CreateShaderModuleFromWords(device, vertWords);
  fragMod := CreateShaderModuleFromWords(device, fragWords);
  ...
  b.pipelineLayout := 0;
  VkCheck(vkCreatePipelineLayout(device, @plci, nil, b.pipelineLayout), 'vkCreatePipelineLayout');
...
begin
  ...
  CreateSwapchainBundle(gInstance, gDevice, gPhysicalDevice, gSurface, gHwnd, gGraphicsQFamily, gPresentQFamily,
	  'hello.vert', 'hello.frag', newBundle);
...
const
  CLASS_NAME = 'VkTriangleWindowPascal';
var
  wc: WNDCLASSEX;
begin
...
begin
  ...
  app.pApplicationName := 'VkTrianglePascal';
