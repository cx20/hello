...
(*
  Vulkan 1.4 Raymarching (Windows, Free Pascal, no external Pascal libs)

  - Win32 window (Windows unit)
  - Vulkan loaded dynamically from vulkan-1.dll
  - Swapchain + render pass + graphics pipeline
  - Fullscreen triangle using gl_VertexIndex (no vertex buffer)
  - SPIR-V shaders loaded from hello_vert.spv / hello_frag.spv
...
begin
  ...
  pc.iTime := timeSec;
  pc.padding := 0.0;
  pc.iResolution[0] := b.extent.width;
  pc.iResolution[1] := b.extent.height;
  ...
  vkCmdPushConstants(b.commandBuffers[imageIndex], b.pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, SizeOf(pc), @pc);
  vkCmdSetViewport(b.commandBuffers[imageIndex], 0, 1, @vp);
  vkCmdSetScissor(b.commandBuffers[imageIndex], 0, 1, @sc);
  vkCmdDraw(b.commandBuffers[imageIndex], 3, 1, 0, 0);
...
begin
  ...
  pcr.offset := 0;
  pcr.size := SizeOf(TPushConstants);
...
begin
  ...
  RecordCommandBuffer(gDevice, gBundle, imageIndex, (GetTickCount64 - gStartTick) / 1000.0);
  cmdArr[0] := cmd;
...
begin
  ...
  app.pApplicationName := 'VkRaymarchingPascal';
