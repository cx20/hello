#ifndef _WIN32_WINNT
#define _WIN32_WINNT 0x0A00
#endif

#include <afxwin.h>
#include <initguid.h>
#include <roapi.h>
#include <Windows.ui.notifications.h>

DEFINE_GUID(UIID_IToastNotificationManagerStatics, 0x50ac103f, 0xd235, 0x4598, 0xbb, 0xef, 0x98, 0xfe, 0x4d, 0x1a, 0x3a, 0xd4);
DEFINE_GUID(UIID_IToastNotificationFactory,        0x04124b20, 0x82c6, 0x4229, 0xb1, 0x09, 0xfd, 0x9e, 0xd4, 0x66, 0x2b, 0x53);
DEFINE_GUID(UIID_IXmlDocument,                     0xf7f3a506, 0x1e87, 0x42d6, 0xbc, 0xfb, 0xb8, 0xc8, 0x09, 0xfa, 0x54, 0x94);
DEFINE_GUID(UIID_IXmlDocumentIO,                   0x6cd0e74e, 0xee65, 0x4489, 0x9e, 0xbf, 0xca, 0x43, 0xe8, 0x7b, 0xa6, 0x37);

#define APP_ID L"0123456789ABCDEF" // Dummy App ID for this sample

template <class T>
void SafeRelease(T*& p)
{
    if (p != NULL)
    {
        p->Release();
        p = NULL;
    }
}

HRESULT CreateXmlDocumentFromString(
    const wchar_t* xmlString,
    __x_ABI_CWindows_CData_CXml_CDom_CIXmlDocument** doc)
{
    if (doc == NULL || xmlString == NULL)
    {
        return E_INVALIDARG;
    }

    *doc = NULL;

    HSTRING_HEADER xmlDocClassHeader;
    HSTRING xmlDocClassName;
    HRESULT hr = WindowsCreateStringReference(
        RuntimeClass_Windows_Data_Xml_Dom_XmlDocument,
        (UINT32)wcslen(RuntimeClass_Windows_Data_Xml_Dom_XmlDocument),
        &xmlDocClassHeader,
        &xmlDocClassName);
    if (FAILED(hr))
    {
        return hr;
    }

    IInspectable* inspectable = NULL;
    hr = RoActivateInstance(xmlDocClassName, &inspectable);
    if (FAILED(hr))
    {
        return hr;
    }

    hr = inspectable->QueryInterface(UIID_IXmlDocument, (void**)doc);
    SafeRelease(inspectable);
    if (FAILED(hr))
    {
        return hr;
    }

    __x_ABI_CWindows_CData_CXml_CDom_CIXmlDocumentIO* docIO = NULL;
    hr = (*doc)->QueryInterface(UIID_IXmlDocumentIO, (void**)&docIO);
    if (FAILED(hr))
    {
        SafeRelease(*doc);
        return hr;
    }

    HSTRING_HEADER xmlHeader;
    HSTRING xmlHString;
    hr = WindowsCreateStringReference(
        xmlString,
        (UINT32)wcslen(xmlString),
        &xmlHeader,
        &xmlHString);
    if (SUCCEEDED(hr))
    {
        hr = docIO->LoadXml(xmlHString);
    }

    SafeRelease(docIO);
    if (FAILED(hr))
    {
        SafeRelease(*doc);
    }

    return hr;
}

HRESULT ShowToast()
{
    // Initialize WinRT for this thread.
    HRESULT hr = RoInitialize(RO_INIT_MULTITHREADED);
    if (FAILED(hr) && hr != S_FALSE)
    {
        return hr;
    }

    bool needUninitialize = (hr == S_OK || hr == S_FALSE);

    HSTRING_HEADER appIdHeader;
    HSTRING appId;
    hr = WindowsCreateStringReference(APP_ID, (UINT32)wcslen(APP_ID), &appIdHeader, &appId);
    if (FAILED(hr))
    {
        if (needUninitialize) RoUninitialize();
        return hr;
    }

    __x_ABI_CWindows_CData_CXml_CDom_CIXmlDocument* inputXml = NULL;
    hr = CreateXmlDocumentFromString(
        L"<toast activationType=\"protocol\" launch=\"imsprevn://0\" duration=\"long\">\r\n"
        L"    <visual>\r\n"
        L"        <binding template=\"ToastGeneric\">\r\n"
        L"            <text><![CDATA[Hello, WinRT(MFC) World!]]></text>\r\n"
        L"        </binding>\r\n"
        L"    </visual>\r\n"
        L"    <audio src=\"ms-winsoundevent:Notification.Mail\" loop=\"false\" />\r\n"
        L"</toast>\r\n",
        &inputXml);
    if (FAILED(hr))
    {
        if (needUninitialize) RoUninitialize();
        return hr;
    }

    HSTRING_HEADER toastMgrHeader;
    HSTRING toastMgrClass;
    hr = WindowsCreateStringReference(
        RuntimeClass_Windows_UI_Notifications_ToastNotificationManager,
        (UINT32)wcslen(RuntimeClass_Windows_UI_Notifications_ToastNotificationManager),
        &toastMgrHeader,
        &toastMgrClass);
    if (FAILED(hr))
    {
        SafeRelease(inputXml);
        if (needUninitialize) RoUninitialize();
        return hr;
    }

    __x_ABI_CWindows_CUI_CNotifications_CIToastNotificationManagerStatics* toastStatics = NULL;
    hr = RoGetActivationFactory(
        toastMgrClass,
        UIID_IToastNotificationManagerStatics,
        (LPVOID*)&toastStatics);
    if (FAILED(hr))
    {
        SafeRelease(inputXml);
        if (needUninitialize) RoUninitialize();
        return hr;
    }

    __x_ABI_CWindows_CUI_CNotifications_CIToastNotifier* notifier = NULL;
    hr = toastStatics->CreateToastNotifierWithId(appId, &notifier);
    if (FAILED(hr))
    {
        SafeRelease(toastStatics);
        SafeRelease(inputXml);
        if (needUninitialize) RoUninitialize();
        return hr;
    }

    HSTRING_HEADER toastHeader;
    HSTRING toastClass;
    hr = WindowsCreateStringReference(
        RuntimeClass_Windows_UI_Notifications_ToastNotification,
        (UINT32)wcslen(RuntimeClass_Windows_UI_Notifications_ToastNotification),
        &toastHeader,
        &toastClass);

    __x_ABI_CWindows_CUI_CNotifications_CIToastNotificationFactory* notifFactory = NULL;
    __x_ABI_CWindows_CUI_CNotifications_CIToastNotification* toast = NULL;

    if (SUCCEEDED(hr))
    {
        hr = RoGetActivationFactory(
            toastClass,
            UIID_IToastNotificationFactory,
            (LPVOID*)&notifFactory);
    }

    if (SUCCEEDED(hr))
    {
        hr = notifFactory->CreateToastNotification(inputXml, &toast);
    }

    if (SUCCEEDED(hr))
    {
        hr = notifier->Show(toast);
    }

    SafeRelease(toast);
    SafeRelease(notifFactory);
    SafeRelease(notifier);
    SafeRelease(toastStatics);
    SafeRelease(inputXml);

    if (needUninitialize)
    {
        RoUninitialize();
    }

    return hr;
}

extern "C" int WINAPI WinMain(
    HINSTANCE hInstance,
    HINSTANCE hPrevInstance,
    LPSTR lpCmdLine,
    int nCmdShow)
{
    UNREFERENCED_PARAMETER(lpCmdLine);

    // Initialize MFC and dispatch control to the CWinApp instance.
    if (!AfxWinInit(hInstance, hPrevInstance, GetCommandLine(), nCmdShow))
    {
        return FALSE;
    }

    CWinApp* app = AfxGetApp();
    if (app == NULL)
    {
        AfxWinTerm();
        return FALSE;
    }

    int exitCode = 0;
    if (!app->InitApplication())
    {
        exitCode = FALSE;
    }
    else if (!app->InitInstance())
    {
        exitCode = app->ExitInstance();
    }
    else
    {
        exitCode = app->Run();
    }

    AfxWinTerm();
    return exitCode;
}

class CHelloApp : public CWinApp
{
public:
    virtual BOOL InitInstance();
};

BOOL CHelloApp::InitInstance()
{
    CWinApp::InitInstance();

    const HRESULT hr = ShowToast();
    if (FAILED(hr))
    {
        CString message;
        message.Format(_T("Failed to show toast notification. HRESULT=0x%08X"), (UINT)hr);
        AfxMessageBox(message, MB_ICONERROR | MB_OK);
    }

    // Exit immediately because this sample only demonstrates toast display.
    return FALSE;
}

CHelloApp theApp;


