...
echo [build] cl /nologo /std:c++17 /EHsc /I"%SDK_CPPWINRT%" /I"%OUT_DIR%" /I"%FOUNDATION_INCLUDE%" /I"%RUNTIME_INCLUDE%" hello.cpp /link /SUBSYSTEM:WINDOWS /LIBPATH:"%FOUNDATION_LIB%" runtimeobject.lib ole32.lib oleaut32.lib windowsapp.lib CoreMessaging.lib Microsoft.WindowsAppRuntime.Bootstrap.lib
cl /nologo /std:c++17 /EHsc /I"%SDK_CPPWINRT%" /I"%OUT_DIR%" /I"%FOUNDATION_INCLUDE%" /I"%RUNTIME_INCLUDE%" hello.cpp /link /SUBSYSTEM:WINDOWS /LIBPATH:"%FOUNDATION_LIB%" runtimeobject.lib ole32.lib oleaut32.lib windowsapp.lib CoreMessaging.lib Microsoft.WindowsAppRuntime.Bootstrap.lib
set "RET=%ERRORLEVEL%"
if "%RET%"=="0" (
