// hello.cpp - OpenGL + D3D11 + Vulkan Triangles via DirectComposition (MFC)
//
// Build (Visual Studio Developer Command Prompt):
//   cl /EHsc /std:c++20 hello.cpp ^
//      /link vulkan-1.lib ^
//      d3d11.lib dxgi.lib d3dcompiler.lib dcomp.lib opengl32.lib ^
//      user32.lib gdi32.lib shell32.lib /SUBSYSTEM:WINDOWS
//
...
//   DirectComposition Visual positioned side-by-side in one MFC window.
//
//   ---------------------- One CFrameWnd --------------------
...
static std::vector<char> ReadSpvFile(LPCTSTR filename) {
    ...
    if (!file.Open(filename, CFile::modeRead | CFile::typeBinary, &ex)) {
        ...
        AfxThrowFileException(ex.m_cause, ex.m_lOsError, filename);
    }
...
// ============================================================
// D3D11 vertex struct for D3D11 panel
// ============================================================
struct DxVertex { float x, y, z, r, g, b, a; };

// ============================================================
// D3D11 embedded HLSL shader for D3D11 panel
// ============================================================
...
// ============================================================
// CMainFrame - MFC main frame window with DirectComposition
// ============================================================
...
private:
    ...
    IDCompositionVisual*  m_pGLVisual;
    CClientDC*            m_pGLDC;      // MFC device context for OpenGL
    HGLRC                 m_hGLRC;
    ...
    IDCompositionVisual*    m_pDXVisual;
    ...
    IDCompositionVisual* m_pVKVisual;
    VkInstance           m_vkInst;
    ...
    // Timer ID for continuous rendering
...
CMainFrame::CMainFrame()
    ...
    , m_pDCompDevice(NULL), m_pDCompTarget(NULL), m_pRootVisual(NULL)
    // OpenGL
    , m_pGLSwapChain(NULL), m_pGLBackBuffer(NULL), m_pGLVisual(NULL)
    ...
    // D3D11
    , m_pDXSwapChain(NULL), m_pDXRTV(NULL), m_pDXVisual(NULL)
    , m_pDXVS(NULL), m_pDXPS(NULL), m_pDXInputLayout(NULL), m_pDXVertexBuffer(NULL)
    // Vulkan
    , m_pVKSwapChain(NULL), m_pVKBackBuffer(NULL), m_pVKStagingTex(NULL), m_pVKVisual(NULL)
    ...
    , m_vkStagBuf(VK_NULL_HANDLE), m_vkStagMem(VK_NULL_HANDLE)
{
    m_nGLVBO[0] = 0;
    m_nGLVBO[1] = 0;
    Create(NULL, _T("OpenGL + D3D11 + Vulkan (DirectComposition / MFC)"));
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    TRACE(_T("================================================\n"));
    TRACE(_T("OpenGL + D3D11 + Vulkan via DirectComposition (MFC)\n"));
    TRACE(_T("3 panels in 1 window, each a separate SwapChain\n"));
    TRACE(_T("================================================\n"));

    if (!CreateD3D11Device())       return -1;
    if (!InitDirectComposition())   return -1;
    ...
    // Commit the visual tree to DWM
    m_pDCompDevice->Commit();
    TRACE(_T("[Main] DComp Commit ok\n"));
...
// ============================================================
// OnPaint - Render all three panels
// ============================================================
...
// ============================================================
// OnEraseBkgnd - Suppress background erase (DComp handles it)
// ============================================================
...
// ============================================================
// RenderAll - Render all three panels
// ============================================================
...
// ============================================================
BOOL CMainFrame::InitDirectComposition()
{
    TRACE(_T("[DComp] begin\n"));
    IDXGIDevice* pDXGIDev = nullptr;
    m_pd3dDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&pDXGIDev);
    DCompositionCreateDevice(pDXGIDev, __uuidof(IDCompositionDevice), (void**)&m_pDCompDevice);
    pDXGIDev->Release();

    m_pDCompDevice->CreateTargetForHwnd(m_hWnd, TRUE, &m_pDCompTarget);

    // Root visual (container)
    m_pDCompDevice->CreateVisual(&m_pRootVisual);
    m_pDCompTarget->SetRoot(m_pRootVisual);

    // Panel 0: OpenGL (left)
    m_pDCompDevice->CreateVisual(&m_pGLVisual);
    m_pGLVisual->SetOffsetX(0.0f);
    m_pGLVisual->SetOffsetY(0.0f);

    // Panel 1: D3D11 (center)
    m_pDCompDevice->CreateVisual(&m_pDXVisual);
    m_pDXVisual->SetOffsetX((float)PANEL_W);
    m_pDXVisual->SetOffsetY(0.0f);

    // Panel 2: Vulkan (right)
    m_pDCompDevice->CreateVisual(&m_pVKVisual);
    m_pVKVisual->SetOffsetX((float)(PANEL_W * 2));
    m_pVKVisual->SetOffsetY(0.0f);

    // Build visual tree: root -> gl -> dx -> vk (sibling chain)
    m_pRootVisual->AddVisual(m_pGLVisual, TRUE, nullptr);
    m_pRootVisual->AddVisual(m_pDXVisual, TRUE, m_pGLVisual);
    m_pRootVisual->AddVisual(m_pVKVisual, TRUE, m_pDXVisual);

    TRACE(_T("[DComp] 3 visuals created\n"));
    return TRUE;
}
...
BOOL CMainFrame::InitOpenGL()
{
    ...
    // Create temporary GL context to get extension functions
    ...
    m_pGLVisual->SetContent(m_pGLSwapChain);

    // Create GL shaders and geometry
    m_pfnGenVertexArrays(1, &m_nGLVAO);
...
BOOL CMainFrame::InitD3D11Panel()
{
    ...
    m_pDXVisual->SetContent(m_pDXSwapChain);
...
BOOL CMainFrame::InitVulkan()
{
    ...
    // Pipeline - load SPIR-V shaders from hello_vert.spv / hello_frag.spv
    ...
    m_pVKVisual->SetContent(m_pVKSwapChain);
    TRACE(_T("[VK] ok\n"));
    return TRUE;
}
...
void CMainFrame::CleanupAll()
{
    ...
    // DComp
    SafeRelease(&m_pVKVisual);
    SafeRelease(&m_pDXVisual);
    SafeRelease(&m_pGLVisual);
