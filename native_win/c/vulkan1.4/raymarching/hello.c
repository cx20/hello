...
static void initWindow(App *app) {
    ...
    app->window = glfwCreateWindow(WIDTH, HEIGHT, "Hello Vulkan Ray Marching (C)", NULL, NULL);
...
static void recordCommandBuffer(App *app, VkCommandBuffer cmd, uint32_t imageIndex, const PushConstants *pc) {
    ...
    rp.framebuffer = app->swapChainFramebuffers[imageIndex];
