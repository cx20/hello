 ...
 *   - DirectX 11 rendering (dynamic texture + fullscreen quad)
 *   - Player 1 keyboard input
 *
 * Build (MSVC):
 *   cl /O2 /W3 /D_CRT_SECURE_NO_WARNINGS hello.c /link user32.lib d3d11.lib d3dcompiler.lib dxguid.lib
 *
 * Usage:
 *   hello.exe game.nes
 */
...
static void render_release_target(Renderer *r) {
    if (r->render_target_view) {
        r->render_target_view->lpVtbl->Release(r->render_target_view);
        r->render_target_view = NULL;
    }
}
...
static int render_create_target(Renderer *r, HWND hwnd) {
    ...
    hr = r->swap_chain->lpVtbl->ResizeBuffers(
        r->swap_chain, 0,
        (UINT)(rc.right - rc.left),
        (UINT)(rc.bottom - rc.top),
        DXGI_FORMAT_UNKNOWN, 0);
    if (FAILED(hr) && hr != DXGI_ERROR_INVALID_CALL) return 0;

    hr = r->swap_chain->lpVtbl->GetBuffer(
        r->swap_chain, 0, (REFIID)&IID_ID3D11Texture2D, (void **)&back_buffer);
    if (FAILED(hr)) return 0;

    hr = r->device->lpVtbl->CreateRenderTargetView(
        r->device, (ID3D11Resource *)back_buffer, NULL, &r->render_target_view);
    back_buffer->lpVtbl->Release(back_buffer);
    if (FAILED(hr)) return 0;

    r->context->lpVtbl->OMSetRenderTargets(r->context, 1, &r->render_target_view, NULL);
...
static int render_init(Renderer *r, HWND hwnd) {
    ...
    sd.BufferCount = 1;
    sd.BufferDesc.Width = (UINT)(rc.right - rc.left);
    sd.BufferDesc.Height = (UINT)(rc.bottom - rc.top);
    ...
    for (i = 0; i < ARRAYSIZE(driver_types); i++) {
        hr = D3D11CreateDeviceAndSwapChain(
            NULL, driver_types[i], NULL, 0, feature_levels, ARRAYSIZE(feature_levels),
            D3D11_SDK_VERSION, &sd, &r->swap_chain, &r->device, NULL, &r->context);
        if (SUCCEEDED(hr)) break;
    }
    if (FAILED(hr)) return 0;
    if (!render_create_target(r, hwnd)) return 0;

    hr = compile_shader_from_file(L"hello.fx", "VS", "vs_4_0", &vs_blob);
    if (FAILED(hr)) return 0;
    hr = r->device->lpVtbl->CreateVertexShader(
        r->device, vs_blob->lpVtbl->GetBufferPointer(vs_blob),
        vs_blob->lpVtbl->GetBufferSize(vs_blob), NULL, &r->vertex_shader);
    if (FAILED(hr)) { vs_blob->lpVtbl->Release(vs_blob); return 0; }
    hr = r->device->lpVtbl->CreateInputLayout(
        r->device, layout, ARRAYSIZE(layout),
        vs_blob->lpVtbl->GetBufferPointer(vs_blob),
        vs_blob->lpVtbl->GetBufferSize(vs_blob), &r->input_layout);
    vs_blob->lpVtbl->Release(vs_blob);
    if (FAILED(hr)) return 0;

    hr = compile_shader_from_file(L"hello.fx", "PS", "ps_4_0", &ps_blob);
    if (FAILED(hr)) return 0;
    hr = r->device->lpVtbl->CreatePixelShader(
        r->device, ps_blob->lpVtbl->GetBufferPointer(ps_blob),
        ps_blob->lpVtbl->GetBufferSize(ps_blob), NULL, &r->pixel_shader);
    ...
    hr = r->device->lpVtbl->CreateBuffer(r->device, &bd, &init_data, &r->vertex_buffer);
    if (FAILED(hr)) return 0;

    memset(&td, 0, sizeof(td));
    td.Width = NES_WIDTH;
    td.Height = NES_HEIGHT;
    td.MipLevels = 1;
    td.ArraySize = 1;
    ...
    hr = r->device->lpVtbl->CreateTexture2D(r->device, &td, NULL, &r->texture);
    if (FAILED(hr)) return 0;
    ...
    hr = r->device->lpVtbl->CreateShaderResourceView(
        r->device, (ID3D11Resource *)r->texture, &srvd, &r->shader_resource_view);
    if (FAILED(hr)) return 0;
    ...
    hr = r->device->lpVtbl->CreateSamplerState(r->device, &sampd, &r->sampler_state);
    if (FAILED(hr)) return 0;
...
static void render_destroy(Renderer *r) {
    if (r->context) r->context->lpVtbl->ClearState(r->context);
    render_release_target(r);
    if (r->sampler_state) r->sampler_state->lpVtbl->Release(r->sampler_state);
    if (r->shader_resource_view) r->shader_resource_view->lpVtbl->Release(r->shader_resource_view);
    if (r->texture) r->texture->lpVtbl->Release(r->texture);
    if (r->vertex_buffer) r->vertex_buffer->lpVtbl->Release(r->vertex_buffer);
    if (r->input_layout) r->input_layout->lpVtbl->Release(r->input_layout);
    if (r->vertex_shader) r->vertex_shader->lpVtbl->Release(r->vertex_shader);
    if (r->pixel_shader) r->pixel_shader->lpVtbl->Release(r->pixel_shader);
...
static void render_resize(Renderer *r, HWND hwnd) {
    if (r->context) {
        ...
        r->context->lpVtbl->OMSetRenderTargets(r->context, 1, &null_rtv, NULL);
    }
...
static void render_frame(Renderer *r, HWND hwnd, const uint32_t *fb) {
    ...
    (void)hwnd;
    if (!r->context || !r->texture || !r->render_target_view) return;

    if (FAILED(r->context->lpVtbl->Map(
            r->context, (ID3D11Resource *)r->texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
        return;
    }
    for (y = 0; y < NES_HEIGHT; y++) {
        memcpy((uint8_t *)mapped.pData + mapped.RowPitch * y,
               fb + y * NES_WIDTH,
               NES_WIDTH * sizeof(uint32_t));
    }
    r->context->lpVtbl->Unmap(r->context, (ID3D11Resource *)r->texture, 0);

    r->context->lpVtbl->OMSetRenderTargets(r->context, 1, &r->render_target_view, NULL);
    r->context->lpVtbl->ClearRenderTargetView(r->context, r->render_target_view, clear_color);
    r->context->lpVtbl->IASetInputLayout(r->context, r->input_layout);
    r->context->lpVtbl->IASetVertexBuffers(r->context, 0, 1, &r->vertex_buffer, &stride, &offset);
    r->context->lpVtbl->IASetPrimitiveTopology(r->context, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
    r->context->lpVtbl->VSSetShader(r->context, r->vertex_shader, NULL, 0);
    r->context->lpVtbl->PSSetShader(r->context, r->pixel_shader, NULL, 0);
    r->context->lpVtbl->PSSetShaderResources(r->context, 0, 1, &r->shader_resource_view);
    r->context->lpVtbl->PSSetSamplers(r->context, 0, 1, &r->sampler_state);
    r->context->lpVtbl->Draw(r->context, 4, 0);
