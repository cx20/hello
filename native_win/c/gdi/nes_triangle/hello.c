 ...
 *
 * Build (MSVC):
 *   cl /O2 /W3 /D_CRT_SECURE_NO_WARNINGS hello.c /link user32.lib gdi32.lib
 *
 * Usage:
 *   hello.exe game.nes
 */
...
static int render_init(Renderer *r, HWND hwnd) {
    ...
    bmi.bmiHeader.biWidth       = NES_WIDTH;
    bmi.bmiHeader.biHeight      = -NES_HEIGHT;
    bmi.bmiHeader.biPlanes      = 1;
    bmi.bmiHeader.biBitCount    = 32;
    bmi.bmiHeader.biCompression = BI_RGB;
    hdc = GetDC(hwnd);
    r->hdc_mem = CreateCompatibleDC(hdc);
    r->hbm = CreateDIBSection(hdc, &bmi, DIB_RGB_COLORS, (void**)&r->pixels, NULL, 0);
    ReleaseDC(hwnd, hdc);
    if (!r->hbm || !r->pixels) return 0;
    r->hbm_old = (HBITMAP)SelectObject(r->hdc_mem, r->hbm);
    return 1;
}

static void render_destroy(Renderer *r) {
    if (r->hdc_mem) { if (r->hbm_old) SelectObject(r->hdc_mem, r->hbm_old); DeleteDC(r->hdc_mem); }
    if (r->hbm) DeleteObject(r->hbm);
}

static void render_frame(Renderer *r, HWND hwnd, const uint32_t *fb) {
    for (int i = 0; i < NES_WIDTH*NES_HEIGHT; i++) {
        uint32_t c = fb[i];
        r->pixels[i] = ((c>>16)&0xFF)<<16 | ((c>>8)&0xFF)<<8 | (c&0xFF);
    }
    HDC hdc = GetDC(hwnd);
    StretchBlt(hdc, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT,
               r->hdc_mem, 0, 0, NES_WIDTH, NES_HEIGHT, SRCCOPY);
    ReleaseDC(hwnd, hdc);
}
...
static LRESULT CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_PAINT: {
        ...
        StretchBlt(hdc,0,0,WINDOW_WIDTH,WINDOW_HEIGHT,
                   g_renderer.hdc_mem,0,0,NES_WIDTH,NES_HEIGHT,SRCCOPY);
...
int main(int argc, char *argv[]) {
    ...
    if (!cartridge_load(&g_cart, argv[1])) return 1;
