 ...
 * CreateSwapChainForComposition, then show three swapchains via DirectComposition
 * on a classic desktop app.
 *
 * Pure C implementation using raw COM vtable calls (including DirectComposition).
 *
 * Logging: DebugView only (OutputDebugStringW). No console, no MessageBox.
 *
 * Build example (MSVC, adjust Windows Kits path if needed):
 *   cl hello.c ^
 *     /link user32.lib gdi32.lib opengl32.lib d3d11.lib dxguid.lib d3dcompiler.lib dcomp.lib ^
 *     /SUBSYSTEM:WINDOWS
 */
...
/* ============================================================
 * DirectComposition (raw COM vtable calls)
 * ============================================================ */
...
static const IID IID_IDCompositionDevice_C =
    { 0xC37EA93A, 0xE7AA, 0x450D, { 0xB1, 0x6F, 0x97, 0x46, 0xCB, 0x04, 0x07, 0xF3 } };

typedef HRESULT (STDMETHODCALLTYPE *PFN_DCCommit)(void* pThis);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCCreateTargetForHwnd)(
    void* pThis, HWND hwnd, BOOL topmost, void** ppTarget);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCCreateVisual)(void* pThis, void** ppVisual);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCTargetSetRoot)(void* pThis, void* pRootVisual);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCVisualSetOffsetX)(void* pThis, float offsetX);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCVisualSetOffsetY)(void* pThis, float offsetY);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCVisualSetContent)(void* pThis, IUnknown* pContent);
typedef HRESULT (STDMETHODCALLTYPE *PFN_DCVisualAddVisual)(
    void* pThis, void* visual, BOOL insertAbove, void* referenceVisual);

static HRESULT DCompDevice_Commit(void* dcompDevice)
{
    return ((PFN_DCCommit)SLOT(dcompDevice, 3))(dcompDevice);
}

static HRESULT DCompDevice_CreateTargetForHwnd(
    void* dcompDevice, HWND hwnd, BOOL topmost, void** ppTarget)
{
    return ((PFN_DCCreateTargetForHwnd)SLOT(dcompDevice, 6))(dcompDevice, hwnd, topmost, ppTarget);
}

static HRESULT DCompDevice_CreateVisual(void* dcompDevice, void** ppVisual)
{
    return ((PFN_DCCreateVisual)SLOT(dcompDevice, 7))(dcompDevice, ppVisual);
}

static HRESULT DCompTarget_SetRoot(void* dcompTarget, void* rootVisual)
{
    return ((PFN_DCTargetSetRoot)SLOT(dcompTarget, 3))(dcompTarget, rootVisual);
}

static HRESULT DCompVisual_SetOffsetX(void* visual, float x)
{
    return ((PFN_DCVisualSetOffsetX)SLOT(visual, 4))(visual, x);
}

static HRESULT DCompVisual_SetOffsetY(void* visual, float y)
{
    return ((PFN_DCVisualSetOffsetY)SLOT(visual, 6))(visual, y);
}

static HRESULT DCompVisual_SetContent(void* visual, IUnknown* content)
{
    return ((PFN_DCVisualSetContent)SLOT(visual, 15))(visual, content);
}

static HRESULT DCompVisual_AddVisual(void* parent, void* child, BOOL insertAbove, void* referenceVisual)
{
    return ((PFN_DCVisualAddVisual)SLOT(parent, 16))(parent, child, insertAbove, referenceVisual);
}
...
static HRESULT CreateAppWindow(HINSTANCE hInst)
{
    ...
    g_hwnd = CreateWindowExW(
        ...
        L"OpenGL + D3D11 + Vulkan via DirectComposition (C)",
        style,
    ...
    void** ppVisual)
{
    ...
    /* Not used by DirectComposition path. Keep outputs null for cleanup safety. */
    *ppSurface = NULL;
    *ppSurfaceBrush = NULL;
    *ppCompositionBrush = NULL;
    *ppSpriteVisual = NULL;
    *ppVisual = NULL;

    if (!g_compositor || !g_rootVisual)
        return E_FAIL;
    ...
    hr = DCompVisual_SetContent(dcompVisual, (IUnknown*)sc);
    if (FAILED(hr)) { Rel(dcompVisual); return hr; }

    hr = DCompVisual_AddVisual(g_rootVisual, dcompVisual, TRUE, NULL);
    if (FAILED(hr)) { Rel(dcompVisual); return hr; }

    *ppVisual = dcompVisual; /* caller releases it during cleanup */
    return S_OK;
}
 ...
 *
 * This creates an IDCompositionDevice / target / root visual and
 * attaches the left (OpenGL) swapchain directly as visual content.
 * ============================================================ */
static HRESULT InitCompositionForHwnd(void)
{
    ...
    if (FAILED(hr))
    {
        dbg_hr(FN, L"QI(ID3D11Device->IDXGIDevice)", hr);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        dbg_hr(FN, L"DCompositionCreateDevice", hr);
        return hr;
    }
    dbg_step(FN, L"IDCompositionDevice created");

    hr = DCompDevice_CreateTargetForHwnd(g_compositor, g_hwnd, TRUE, &g_compositionTarget);
    if (FAILED(hr))
    {
        dbg_hr(FN, L"CreateTargetForHwnd", hr);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        dbg_hr(FN, L"CreateVisual(root)", hr);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        dbg_hr(FN, L"CreateVisual(GL panel)", hr);
        return hr;
    }
    hr = DCompVisual_SetOffsetX(glVisual, 0.0f);
    if (FAILED(hr)) { Rel(glVisual); dbg_hr(FN, L"GL SetOffsetX", hr); return hr; }
    hr = DCompVisual_SetOffsetY(glVisual, 0.0f);
    if (FAILED(hr)) { Rel(glVisual); dbg_hr(FN, L"GL SetOffsetY", hr); return hr; }
    hr = DCompVisual_SetContent(glVisual, (IUnknown*)g_swapChain);
    if (FAILED(hr)) { Rel(glVisual); dbg_hr(FN, L"GL SetContent", hr); return hr; }
    hr = DCompVisual_AddVisual(g_rootVisual, glVisual, TRUE, NULL);
    if (FAILED(hr)) { Rel(glVisual); dbg_hr(FN, L"Root AddVisual(GL)", hr); return hr; }

    g_visual = glVisual; /* released in Cleanup() */
    dbg_step(FN, L"DirectComposition visual tree initialized");
    return S_OK;
}
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE hPrev, PWSTR pCmdLine, int nCmdShow)
{
    ...
    if (g_compositor)
    {
        ...
        if (FAILED(hr))
        {
            dbgprintf(L"[FATAL] DComp Commit failed hr=0x%08X\n", (uint32_t)hr);
            Cleanup();
            return (int)hr;
        }
    }
