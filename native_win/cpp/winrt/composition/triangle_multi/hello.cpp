...
// GL function pointer types (abbreviated names for brevity)
...
static void InitComposition() {
    ...
    if (FAILED(hr)) throw std::runtime_error("RoInitialize failed");
    dbg("[WUC] RoInitialize ok\n");

    // Step 2: Create DispatcherQueueController (required for Compositor)
    // Dynamically load from CoreMessaging.dll
    ...
    dbg("[WUC] ICompositorDesktopInterop=%p\n", g_compDesktopInt);

    // Step 5: Create DesktopWindowTarget (bind Compositor to HWND)
    ...
    dbg("[WUC] DesktopWindowTarget=%p\n", g_windowTarget);

    // Step 6: Get ICompositionTarget from DesktopWindowTarget
    ...
    // Step 7: Create root ContainerVisual
    ...
    hr = g_rootVisual->put_Size(rootSize);
    ...
    dbg("[WUC] root container set, ready for panels\n");
}
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, LPWSTR, int) {
    ...
    try {
        CreateAppWindow();
        CreateD3D11Device();
        InitComposition();     // WinRT Compositor + DesktopWindowTarget
