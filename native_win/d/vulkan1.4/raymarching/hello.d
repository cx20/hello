...
extern (Windows) int QueryPerformanceFrequency(LARGE_INTEGER* lpFrequency);
extern (Windows) int QueryPerformanceCounter(LARGE_INTEGER* lpPerformanceCount);
...
extern(System) {
    ...
    alias PFN_vkCmdPushConstants = void function(VkCommandBuffer, VkPipelineLayout, VkShaderStageFlags, uint, uint, const(void)*);
...
bool loadVulkan() {
    ...
    DebugLog("loadVulkan: Loading vulkan-1.dll");
    ...
    DebugLog("loadVulkan: vulkan-1.dll loaded successfully");

    DebugLog("loadVulkan: Getting vkGetInstanceProcAddr");
    ...
    DebugLog("loadVulkan: vkGetInstanceProcAddr loaded successfully");

    // Load global functions
    DebugLog("loadVulkan: Loading global functions");
    ...
    DebugLog("loadVulkan: Global functions loaded");

    DebugLog("loadVulkan: Completed successfully");
    return true;
}
...
void createInstance() {
    ...
    DebugLog("createInstance: ApplicationInfo configured");
    ...
    DebugLog("createInstance: Extensions and layers configured");
    ...
    DebugLog("createInstance: Debug messenger configured");
    ...
    DebugLog("createInstance: CreateInfo prepared, calling vkCreateInstance");

    VkResult result = vkCreateInstance(&createInfo, null, &gApp.instance);
    if (result != VK_SUCCESS) {
        DebugLog(format("createInstance: vkCreateInstance failed with result %d", result));
        printf("Failed to create instance! Result: %d\n", result);
        exit(1);
    }
    DebugLog("createInstance: Instance created successfully");

    DebugLog("createInstance: Loading instance functions");
    loadInstanceFunctions(gApp.instance);
    DebugLog("createInstance: Instance functions loaded");

    if (ENABLE_VALIDATION && vkCreateDebugUtilsMessengerEXT !is null) {
        DebugLog("createInstance: Creating debug messenger");
        vkCreateDebugUtilsMessengerEXT(gApp.instance, &debugCreateInfo, null, &gApp.debugMessenger);
        DebugLog("createInstance: Debug messenger created");
    }
    
    DebugLog("createInstance: Completed successfully");
}
...
void createSurface() {
    ...
    DebugLog("createSurface: Calling vkCreateWin32SurfaceKHR");
    VkResult result = vkCreateWin32SurfaceKHR(gApp.instance, &createInfo, null, &gApp.surface);
    if (result != VK_SUCCESS) {
        DebugLog(format("createSurface: vkCreateWin32SurfaceKHR failed with result %d", result));
        printf("Failed to create window surface! Result: %d\n", result);
        exit(1);
    }
    
    DebugLog("createSurface: Completed successfully");
}
...
void pickPhysicalDevice() {
    ...
    uint deviceCount = 0;
    DebugLog("pickPhysicalDevice: Enumerating physical devices (pass 1)");
    vkEnumeratePhysicalDevices(gApp.instance, &deviceCount, null);
    DebugLog(format("pickPhysicalDevice: Found %d devices", deviceCount));
    
    if (deviceCount == 0) {
        DebugLog("pickPhysicalDevice: No Vulkan devices found!");
        printf("No Vulkan devices found!\n");
        exit(1);
    }

    auto devices = allocArray!VkPhysicalDevice(deviceCount);
    DebugLog("pickPhysicalDevice: Enumerating physical devices (pass 2)");
    vkEnumeratePhysicalDevices(gApp.instance, &deviceCount, devices);

    // Use first device
    gApp.physicalDevice = devices[0];
    DebugLog("pickPhysicalDevice: Selected first device");

    VkPhysicalDeviceProperties props;
    vkGetPhysicalDeviceProperties(gApp.physicalDevice, &props);
    DebugLog(format("pickPhysicalDevice: Device name: %s", props.deviceName[0..20]));
    ...
    DebugLog("pickPhysicalDevice: Getting queue family count");
    vkGetPhysicalDeviceQueueFamilyProperties(gApp.physicalDevice, &queueFamilyCount, null);
    DebugLog(format("pickPhysicalDevice: Found %d queue families", queueFamilyCount));
    ...
    for (uint i = 0; i < queueFamilyCount; i++) {
        if ((queueFamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) && !foundGraphics) {
            ...
            foundGraphics = true;
            DebugLog(format("pickPhysicalDevice: Found graphics queue family at index %d", i));
        }
        ...
        if (presentSupport && !foundPresent) {
            ...
            foundPresent = true;
            DebugLog(format("pickPhysicalDevice: Found present queue family at index %d", i));
        }
    ...
    if (!foundGraphics || !foundPresent) {
        DebugLog(format("pickPhysicalDevice: Failed to find suitable queue families! Graphics: %d, Present: %d", foundGraphics, foundPresent));
        printf("Failed to find suitable queue families!\n");
        exit(1);
    }
    
    DebugLog("pickPhysicalDevice: Completed successfully");
}
...
void createLogicalDevice() {
    ...
    if (gApp.graphicsFamily != gApp.presentFamily) {
        ...
        queueCount = 2;
        DebugLog("createLogicalDevice: Graphics and present families are different, using 2 queues");
    } else {
        DebugLog("createLogicalDevice: Graphics and present families are the same, using 1 queue");
    }
    ...
    DebugLog("createLogicalDevice: Calling vkCreateDevice");
    VkResult result = vkCreateDevice(gApp.physicalDevice, &createInfo, null, &gApp.device);
    if (result != VK_SUCCESS) {
        DebugLog(format("createLogicalDevice: vkCreateDevice failed with result %d", result));
        printf("Failed to create logical device! Result: %d\n", result);
        exit(1);
    }
    DebugLog("createLogicalDevice: Device created successfully");

    DebugLog("createLogicalDevice: Loading device functions");
    loadDeviceFunctions(gApp.device);
    DebugLog("createLogicalDevice: Device functions loaded");

    DebugLog("createLogicalDevice: Getting device queues");
    ...
    DebugLog("createLogicalDevice: Device queues obtained");
    
    DebugLog("createLogicalDevice: Completed successfully");
}
...
void createRenderPass() {
    DebugLog("createRenderPass: Starting");
    ...
    DebugLog("createRenderPass: Color attachment configured");
    ...
    DebugLog("createRenderPass: Subpass configured");
    ...
    DebugLog("createRenderPass: Subpass dependency configured");
    ...
    DebugLog("createRenderPass: Calling vkCreateRenderPass");

    VkResult result = vkCreateRenderPass(gApp.device, &renderPassInfo, null, &gApp.renderPass);
    if (result != VK_SUCCESS) {
        DebugLog(format("createRenderPass: vkCreateRenderPass failed with result %d", result));
        printf("Failed to create render pass! Result: %d\n", result);
        exit(1);
    }
    
    DebugLog("createRenderPass: Completed successfully");
}

VkShaderModule createShaderModule(const(ubyte)[] code) {
    DebugLog(format("createShaderModule: Creating shader module from code size %d", code.length));
    
    VkShaderModuleCreateInfo createInfo;
    createInfo.codeSize = code.length;
    createInfo.pCode = cast(const(uint)*)code.ptr;
    DebugLog("createShaderModule: CreateInfo prepared");

    VkShaderModule shaderModule;
    DebugLog("createShaderModule: Calling vkCreateShaderModule");
    VkResult result = vkCreateShaderModule(gApp.device, &createInfo, null, &shaderModule);
    if (result != VK_SUCCESS) {
        DebugLog(format("createShaderModule: vkCreateShaderModule failed with result %d", result));
        printf("Failed to create shader module! Result: %d\n", result);
        exit(1);
    }
    DebugLog("createShaderModule: Shader module created successfully");
    return shaderModule;
}

const(ubyte)[] readFile(const(char)* filename) {
    string filenameStr = filename[0..strlen(filename)].idup;
    DebugLog(format("readFile: Reading file %s", filenameStr));
    try {
        auto data = cast(const(ubyte)[])std.file.read(filenameStr);
        DebugLog(format("readFile: Successfully read %s, size: %d bytes", filenameStr, data.length));
        return data;
    } catch (Exception e) {
        DebugLog(format("readFile: Failed to read %s - %s", filenameStr, e.msg));
...
void createGraphicsPipeline() {
    DebugLog("createGraphicsPipeline: Starting");
    
    DebugLog("createGraphicsPipeline: Reading vertex shader");
    auto vertCode = readFile("hello_vert.spv");
    DebugLog(format("createGraphicsPipeline: Vertex shader loaded, size: %d", vertCode.length));
    
    DebugLog("createGraphicsPipeline: Reading fragment shader");
    auto fragCode = readFile("hello_frag.spv");
    DebugLog(format("createGraphicsPipeline: Fragment shader loaded, size: %d", fragCode.length));
    
    DebugLog("createGraphicsPipeline: Creating vertex shader module");
    VkShaderModule vertShaderModule = createShaderModule(vertCode);
    DebugLog("createGraphicsPipeline: Vertex shader module created");
    
    DebugLog("createGraphicsPipeline: Creating fragment shader module");
    VkShaderModule fragShaderModule = createShaderModule(fragCode);
    DebugLog("createGraphicsPipeline: Fragment shader module created");
    ...
    // Add push constant range for shader parameters
    VkPushConstantRange pushConstantRange;
    pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    pushConstantRange.offset = 0;
    pushConstantRange.size = 16; // iTime (4) + padding (4) + iResolution (8)
    
    pipelineLayoutInfo.pushConstantRangeCount = 1;
    pipelineLayoutInfo.pPushConstantRanges = &pushConstantRange;
    
    DebugLog("createGraphicsPipeline: Creating pipeline layout");

    VkResult result = vkCreatePipelineLayout(gApp.device, &pipelineLayoutInfo, null, &gApp.pipelineLayout);
    if (result != VK_SUCCESS) {
        DebugLog(format("createGraphicsPipeline: vkCreatePipelineLayout failed with result %d", result));
        printf("Failed to create pipeline layout! Result: %d\n", result);
        exit(1);
    }
    DebugLog("createGraphicsPipeline: Pipeline layout created");
    ...
    DebugLog("createGraphicsPipeline: Calling vkCreateGraphicsPipelines");
    result = vkCreateGraphicsPipelines(gApp.device, null, 1, &pipelineInfo, null, &gApp.graphicsPipeline);
    if (result != VK_SUCCESS) {
        DebugLog(format("createGraphicsPipeline: vkCreateGraphicsPipelines failed with result %d", result));
        printf("Failed to create graphics pipeline! Result: %d\n", result);
        exit(1);
    }
    DebugLog("createGraphicsPipeline: Graphics pipeline created");
    ...
    DebugLog("createGraphicsPipeline: Completed successfully");
}
...
void recordCommandBuffer(VkCommandBuffer commandBuffer, uint imageIndex) {
    ...
    float iTime = cast(float)(currentTime.QuadPart - gApp.perfCounterStart.QuadPart) / cast(float)gApp.perfCounterFreq.QuadPart;
    float[2] iResolution = [cast(float)gApp.swapChainExtent.width, cast(float)gApp.swapChainExtent.height];
    
    // Pack push constants: float iTime, float padding, vec2 iResolution
    float[4] pushConstants = [iTime, 0.0f, iResolution[0], iResolution[1]];
    
    vkCmdPushConstants(commandBuffer, gApp.pipelineLayout, 
                      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                      0, 16, pushConstants.ptr);
...
void drawFrame() {
    ...
    if (firstFrame) {
        DebugLog("drawFrame: First frame starting");
        firstFrame = false;
    }
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        DebugLog("drawFrame: VK_ERROR_OUT_OF_DATE_KHR received");
        return;
    }
    ...
    if (vkQueueSubmit(gApp.graphicsQueue, 1, &submitInfo, gApp.inFlightFences[gApp.currentFrame]) != VK_SUCCESS) {
        DebugLog("drawFrame: vkQueueSubmit failed!");
...
void initVulkan() {
    ...
    DebugLog("initVulkan: Creating instance");
    createInstance();
    DebugLog("initVulkan: Instance created");
    
    DebugLog("initVulkan: Creating surface");
    createSurface();
    DebugLog("initVulkan: Surface created");
    
    DebugLog("initVulkan: Picking physical device");
    pickPhysicalDevice();
    DebugLog("initVulkan: Physical device picked");
    
    DebugLog("initVulkan: Creating logical device");
    createLogicalDevice();
    DebugLog("initVulkan: Logical device created");
    
    DebugLog("initVulkan: Creating swap chain");
    createSwapChain();
    DebugLog("initVulkan: Swap chain created");
    
    DebugLog("initVulkan: Creating image views");
    createImageViews();
    DebugLog("initVulkan: Image views created");
    
    DebugLog("initVulkan: Creating render pass");
    createRenderPass();
    DebugLog("initVulkan: Render pass created");
    
    DebugLog("initVulkan: Creating graphics pipeline");
    createGraphicsPipeline();
    DebugLog("initVulkan: Graphics pipeline created");
    
    DebugLog("initVulkan: Creating framebuffers");
    createFramebuffers();
    DebugLog("initVulkan: Framebuffers created");
    
    DebugLog("initVulkan: Creating command pool");
    createCommandPool();
    DebugLog("initVulkan: Command pool created");
    
    DebugLog("initVulkan: Creating command buffers");
    createCommandBuffers();
    DebugLog("initVulkan: Command buffers created");
    
    DebugLog("initVulkan: Creating sync objects");
    createSyncObjects();
    DebugLog("initVulkan: Sync objects created");
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    DebugLog("WinMain: Starting application");
    Runtime.initialize();
    DebugLog("WinMain: Runtime initialized, entering myWinMain");
    int result = myWinMain(hInstance, hPrevInstance, lpCmdLine, nCmdShow);
    DebugLog(format("WinMain: myWinMain returned %d, terminating runtime", result));
    Runtime.terminate();
    DebugLog("WinMain: Runtime terminated, exiting");
    return result;
}
...
int myWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Load Vulkan
    DebugLog("myWinMain: Loading Vulkan");
    if (!loadVulkan()) {
        DebugLog("myWinMain: Failed to load Vulkan");
        return 1;
    }
    DebugLog("myWinMain: Vulkan loaded successfully");

    // Register window class
    DebugLog("myWinMain: Registering window class");
    ...
    if (!RegisterClassEx(&wcex)) {
        DebugLog("myWinMain: Failed to register window class");
        return 1;
    }
    DebugLog("myWinMain: Window class registered");

    // Create window
    DebugLog("myWinMain: Creating window");
    ...
    if (gApp.hwnd is null) {
        DebugLog("myWinMain: Failed to create window");
        return 1;
    }
    DebugLog("myWinMain: Window created successfully");

    ShowWindow(gApp.hwnd, nCmdShow);

    // Initialize Vulkan
    DebugLog("myWinMain: Initializing Vulkan");
    initVulkan();
    DebugLog("myWinMain: Vulkan initialized successfully");

    // Initialize performance counters for timing
    QueryPerformanceFrequency(&gApp.perfCounterFreq);
    QueryPerformanceCounter(&gApp.perfCounterStart);
    DebugLog(format("myWinMain: Performance counter frequency: %d", gApp.perfCounterFreq.QuadPart));
    ...
    while (!quit) {
        if (PeekMessage(&msg, null, 0, 0, PM_REMOVE)) {
            if (msg.message == WM_QUIT) {
                DebugLog("myWinMain: Received WM_QUIT, exiting loop");
                quit = true;
            } else {
            ...
            frameCount++;
            if (frameCount % 60 == 0) {
                DebugLog(format("myWinMain: Rendered %d frames", frameCount));
            }
            drawFrame();
        }
    }
    ...
    // Cleanup
    DebugLog("myWinMain: Cleaning up resources");
    cleanup();
    DebugLog("myWinMain: Cleanup completed");
